...
count(self::$vars)-1;}}define('NETTE',TRUE);define('NETTE_DIR',dirname(__FILE__));define('NETTE_VERSION_ID',20006);define('NETTE_PACKAGE','PHP 5.2 prefixed');interface
IBarPanel{function
getTab();function
getPanel();}interface
IPresenter{function
...
new
NRedirectResponse($url,IHttpResponse::S301_MOVED_PERMANENTLY);}}$params=$request->getParameters();if(!isset($params['callback'])){return;}$params['presenter']=$this;$callback=new
...
new
NInvalidLinkException("Invalid value for persistent parameter '$name' in '{$this->getName()}', expected ".($type==='NULL'?'scalar':$type).".");}if($params[$name]===$meta['def']||($meta['def']===NULL&&is_scalar($params[$name])&&(string)$params[$name]==='')){$params[$name]=NULL;}}}final
function
...
NLink($presenter,$name.self::NAME_SEPARATOR.'submit!',array()));if($this->isSubmitted()){foreach($this->getControls()as$control){if(!$control->isDisabled()){$control->loadHttpData();}}}}parent::attached($presenter);}function
...
__construct(NDIContainer$context=NULL){$this->context=$context;if($context&&$this->invalidLinkMode===NULL){$this->invalidLinkMode=$context->parameters['productionMode']?self::INVALID_LINK_SILENT:self::INVALID_LINK_WARNING;}}final
function
...
function
injectPrimary(NDIContainer$context){$this->context=$context;}final
function
...
NCallback||$data
instanceof
Closure){$this->storage->lock($key);$data=NCallback::create($data)->invokeArgs(array(&$dp));}if($data===NULL){$this->storage->remove($key);}else{$this->storage->write($key,$data,$this->completeDependencies($dp,$data));return$data;}}private
function
...
new
FileNotFoundException("Cannot open file '$file'.");}$count=0;$sql='';while(!feof($handle)){$s=fgets($handle);$sql.=$s;if(substr(rtrim($s),-1)===';'){$connection->exec($sql);$sql='';$count++;}}if(trim($sql)!==''){$connection->exec($sql);$count++;}fclose($handle);return$count;}}class
...
NObject
implements
IReflection{protected$cache;protected$cacheStorage;protected$connection;protected$structure=array();function
...
new
PDOException('Ambiguous joining column in related call.');}}if(!$refresh){throw
new
...
process($sql,$params){$this->params=$params;$this->counter=0;$this->remaining=array();$this->arrayMode='assoc';$sql=NStrings::replace($sql,'~\'.*?\'|".*?"|\?|\b(?:INSERT|REPLACE|UPDATE)\b~si',array($this,'callback'));while($this->counter<count($params)){$sql.=' '.$this->formatValue($params[$this->counter++]);}return
...
function
detectColumnTypes(){if($this->types===NULL){$this->types=array();if($this->connection->getSupplementalDriver()->isSupported(ISupplementalDriver::SUPPORT_COLUMNS_META)){$col=0;while($meta=$this->getColumnMeta($col++)){if(isset($meta['native_type'])){$this->types[$meta['name']]=NDatabaseHelpers::detectType($meta['native_type']);}}}}return$this->types;}function
...
NObject
implements
Iterator,ArrayAccess,Countable{protected$connection;protected$sqlBuilder;protected$name;protected$primary;protected$primarySequence=FALSE;protected$rows;protected$data;protected$referenced=array();protected$referencing=array();protected$referencingPrototype=array();protected$aggregation=array();protected$accessed;protected$prevAccessed;protected$observeCache=FALSE;protected$checkReferenced=FALSE;protected$keys=array();function
__construct($table,NConnection$connection){$this->name=$table;$this->connection=$connection;$this->primary=$connection->getDatabaseReflection()->getPrimary($table);$this->sqlBuilder=new
NSqlBuilder($this);}function
...
NTableRow?$row[$column]->getPrimary():$row[$column];$keys[$key]=TRUE;}if($referenced!==NULL&&array_keys($keys)===array_keys($referenced->rows)){return$referenced;}if($keys){$referenced=$this->createSelectionInstance($table);$referenced->where($referenced->primary,array_keys($keys));}else{$referenced=array();}}return$referenced;}function
...
NSqlBuilder($this);$this->where($this->column,$this->active);$return=parent::update($data);$this->sqlBuilder=$builder;return$return;}function
delete(){$builder=$this->sqlBuilder;$this->sqlBuilder=new
NSqlBuilder($this);$this->where($this->column,$this->active);$return=parent::delete();$this->sqlBuilder=$builder;return$return;}}class
NSqlBuilder
extends
NObject{protected$selection;protected$connection;protected$delimitedTable;protected$select=array();protected$where=array();protected$conditions=array();protected$parameters=array();protected$order=array();protected$limit=NULL;protected$offset=NULL;protected$group='';protected$having='';function
__construct(NTableSelection$selection){$this->selection=$selection;$this->connection=$selection->getConnection();$this->delimitedTable=$this->tryDelimite($selection->getName());}function
setSelection(NTableSelection$selection){$this->selection=$selection;}function
...
instanceof
NTableSelection){$clone=clone$parameters;if(!$clone->getSqlBuilder()->select){$clone->select($clone->primary);}if($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)!=='mysql'){$condition.=' IN ('.$clone->getSql().')';}else{$in=array();foreach($clone
...
new
NServiceCreationException("Parameter $parameter has no type hint, so its value must be specified.");}}while(array_key_exists(++$num,$arguments)){$res[$num]=$arguments[$num];unset($arguments[$num]);$optCount=0;}if($arguments){throw
new
...
new
InvalidArgumentException("Items contain duplication for key '$key'.");}$allowed[$key]=$value;}}$this->items=$items;$this->allowed=$allowed;$this->useKeys=(bool)$useKeys;return$this;}final
function
...
addRule($operation,$message=NULL,$arg=NULL){if($operation===NForm::FLOAT){$this->addFilter(array(__CLASS__,'filterFloat'));}return
...
new
NCompileException("Found multiple macro-attributes $token->name.",0,$token->line);}$htmlNode->macroAttrs[$name]=$token->value;}else{$htmlNode->attrs[$token->name]=TRUE;$this->output.=$token->text;if($token->value){$context=NULL;if(strncasecmp($token->name,'on',2)===0){$context=self::CONTENT_JS;}elseif($token->name==='style'){$context=self::CONTENT_CSS;}$this->setContext($token->value,$context);}}}private
function
...
write($mask){$args=func_get_args();array_shift($args);$word=strpos($mask,'%node.word')===FALSE?NULL:$this->argsTokenizer->fetchWord();$me=$this;$mask=NStrings::replace($mask,'#%escape(\(([^()]*+|(?1))+\))#',new
NCallback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me)).'], EXTR_REFS);
			return $me->escape(substr($m[1], 1, -1));
		')));$mask=NStrings::replace($mask,'#%modify(\(([^()]*+|(?1))+\))#',new
NCallback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me)).'], EXTR_REFS);
			return $me->formatModifiers(substr($m[1], 1, -1));
		')));return
NStrings::replace($mask,'#([,+]\s*)?%(node\.word|node\.array|node\.args|var|raw)(\?)?(\s*\+\s*)?()#',new
NCallback(create_function('$m','extract(NCFix::$vars['.NCFix::uses(array('me'=>$me,'word'=>$word,'args'=>&$args)).'], EXTR_REFS);
...
as$item){if($item=realpath($item)){$disallow[$item]=TRUE;}}$iterator=NFinder::findFiles(is_array($this->acceptFiles)?$this->acceptFiles:preg_split('#[,\s]+#',$this->acceptFiles))->filter(create_function('$file','extract(NCFix::$vars['.NCFix::uses(array('disallow'=>&$disallow)).'], EXTR_REFS);
...
NCallback($callback);return$this;}final
function
...
function
strip($s){return
NStrings::replace($s,'#(</textarea|</pre|</script|^).*?(?=<textarea|<pre|<script|$)#si',new
NCallback(create_function('$m','
				return trim(preg_replace("#[ \\t\\r\\n]+#", " ", $m[0]));
			')));}static
function
indent($s,$level=1,$chars="\t"){if($level>=1){$s=NStrings::replace($s,'#<(textarea|pre).*?</\\1#si',new
NCallback(create_function('$m','
				return strtr($m[0], " \\t\\r\\n", "\\x1F\\x1E\\x1D\\x1A");
