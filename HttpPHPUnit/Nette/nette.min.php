<?php //netteloader=HttpPHPUnit\Nette\Framework

namespace HttpPHPUnit\Nette {use
HttpPHPUnit\Nette;
/**
...
error_reporting(E_ALL|E_STRICT);@set_magic_quotes_runtime(FALSE);iconv_set_encoding('internal_encoding','UTF-8');extension_loaded('mbstring')&&mb_internal_encoding('UTF-8');umask(0);@header('X-Powered-By: Nette Framework');@header('Content-Type: text/html; charset=utf-8');/*define('NETTE',TRUE);define('NETTE_DIR',__DIR__);define('NETTE_VERSION_ID',20006);define('NETTE_PACKAGE','5.3');*/}namespace HttpPHPUnit\Nette\Diagnostics{use
HttpPHPUnit\Nette;interface
IBarPanel{function
getTab();function
getPanel();}}namespace HttpPHPUnit\Nette\Application{use
HttpPHPUnit\Nette;interface
IPresenter{function
...
constructUrl(Request$appRequest,Nette\Http\Url$refUrl);}}namespace HttpPHPUnit\Nette{use
HttpPHPUnit\Nette;interface
IFreezable{function
freeze();function
isFrozen();}}namespace HttpPHPUnit\Nette\ComponentModel{use
HttpPHPUnit\Nette;interface
IComponent{const
...
getComponents($deep=FALSE,$filterType=NULL);}}namespace HttpPHPUnit\Nette\Application\UI{use
HttpPHPUnit\Nette;interface
...
isControlInvalid();}}namespace HttpPHPUnit\Nette\Caching{use
HttpPHPUnit\Nette;interface
IStorage{function
read($key);function
lock($key);function
...
remove($key);function
clean(array$conds);}}namespace HttpPHPUnit\Nette\Caching\Storages{use
HttpPHPUnit\Nette;interface
IJournal{function
write($key,array$dependencies);function
clean(array$conditions);}}namespace HttpPHPUnit\Nette\Config{use
HttpPHPUnit\Nette;interface
IAdapter{function
load($file);function
dump(array$data);}}namespace HttpPHPUnit\Nette\Database{use
HttpPHPUnit\Nette;interface
...
setConnection(Connection$connection);}}namespace HttpPHPUnit\Nette\DI{use
HttpPHPUnit\Nette;interface
IContainer{function
addService($name,$service);function
getService($name);function
removeService($name);function
hasService($name);}}namespace HttpPHPUnit\Nette\Forms{use
HttpPHPUnit\Nette;interface
IControl{function
...
render(Form$form);}}namespace HttpPHPUnit\Nette\Http{use
HttpPHPUnit\Nette;interface
IRequest{const
...
remove($id);function
clean($maxlifetime);}}namespace HttpPHPUnit\Nette\Security{use
HttpPHPUnit\Nette;interface
IUserStorage{const
...
getLogoutReason();}}namespace HttpPHPUnit\Nette\Latte{use
HttpPHPUnit\Nette;interface
IMacro{function
initialize();function
finalize();function
nodeOpened(MacroNode$node);function
nodeClosed(MacroNode$node);}}namespace HttpPHPUnit\Nette\Localization{use
HttpPHPUnit\Nette;interface
ITranslator{function
translate($message,$count=NULL);}}namespace HttpPHPUnit\Nette\Mail{use
HttpPHPUnit\Nette;interface
IMailer{function
send(Message$mail);}}namespace HttpPHPUnit\Nette\Reflection{use
HttpPHPUnit\Nette;interface
IAnnotation{function
__construct(array$values);}}namespace HttpPHPUnit\Nette\Security{use
HttpPHPUnit\Nette;interface
...
IRole{function
getRoleId();}}namespace HttpPHPUnit\Nette\Templating{use
HttpPHPUnit\Nette;interface
ITemplate{function
render();}interface
IFileTemplate
...
getFile();}}namespace HttpPHPUnit\Nette{use
HttpPHPUnit\Nette;class
...
__unset($name){ObjectMixin::remove($this,$name);}}}namespace HttpPHPUnit\Nette\Utils{use
HttpPHPUnit\Nette;final
class
...
function
load(){if(func_num_args()>1){self::$vars=func_get_arg(1);if(self::$vars===TRUE){return include_once func_get_arg(0);}extract(self::$vars);}return include func_get_arg(0);}}}namespace HttpPHPUnit\Nette\Loaders{use
HttpPHPUnit\Nette;abstract
class
AutoLoader
extends
Nette\Object{static
...
function
tryLoad($type);}}namespace HttpPHPUnit\Nette\Diagnostics{use
HttpPHPUnit\Nette;final
class
Helpers{static
function
...
as$i=>$item){if(isset($item['function'])&&$item['function']===end($m)&&isset($item['class'])===isset($m[1])&&(!isset($item['class'])||$item['class']===$m[0]||$m[0]==='*'||is_subclass_of($item['class'],$m[0]))){$index=$i;return$item;}}}}}namespace HttpPHPUnit\Nette\Utils{use
HttpPHPUnit\Nette;class
Html
extends
Nette\Object
...
as$key=>$value){if(is_object($value)){$this->children[$key]=clone$value;}}}}}namespace HttpPHPUnit\Nette\Diagnostics{use
HttpPHPUnit\Nette;final
class
Debugger{public
...
instanceof
Nette\Utils\NeonException&&preg_match('#line (\d+)#',$e->getMessage(),$m)){if($item=Helpers::findTrace($e->getTrace(),'HttpPHPUnit\Nette\Config\Adapters\NeonAdapter::load')){return
array('tab'=>'NEON','panel'=>'<p><b>File:</b> '.Helpers::editorLink($item['args'][0],$m[1]).'&nbsp; <b>Line:</b> '.$m[1].'</p>'.BlueScreen::highlightFile($item['args'][0],$m[1]));}elseif($item=Helpers::findTrace($e->getTrace(),'HttpPHPUnit\Nette\Utils\Neon::decode')){return
...
new
Nette\InvalidStateException('Logging directory is not specified in HttpPHPUnit\Nette\Diagnostics\Debugger::$logDirectory.');}if($message
...
FALSE;}$item=array('name'=>'PHP','level'=>$priority,'order'=>count(self::$payload['logs']),'time'=>str_pad(number_format((microtime(TRUE)-Debugger::$time)*1000,1,'.',' '),8,'0',STR_PAD_LEFT).' ms','template'=>'','message'=>'','style'=>'background:#767ab6');$args=func_get_args();if(isset($args[0])&&is_string($args[0])){$item['template']=array_shift($args);}if(isset($args[0])&&$args[0]instanceof\Exception){$e=array_shift($args);$trace=$e->getTrace();if(isset($trace[0]['class'])&&$trace[0]['class']==='HttpPHPUnit\Nette\Diagnostics\Debugger'&&($trace[0]['function']==='_shutdownHandler'||$trace[0]['function']==='_errorHandler')){unset($trace[0]);}$file=str_replace(dirname(dirname(dirname($e->getFile()))),"\xE2\x80\xA6",$e->getFile());$item['template']=($e
instanceof\ErrorException?'':get_class($e).': ').$e->getMessage().($e->getCode()?' #'.$e->getCode():'').' in '.$file.':'.$e->getLine();$item['pathname']=$e->getFile();$item['lineno']=$e->getLine();}else{$trace=debug_backtrace();if(isset($trace[1]['class'])&&$trace[1]['class']==='HttpPHPUnit\Nette\Diagnostics\Debugger'&&($trace[1]['function']==='fireLog')){unset($trace[0]);}foreach($trace
...
instanceof
Nette\FatalErrorException&&isset($errorTypes[$exception->getSeverity()]))?$errorTypes[$exception->getSeverity()]:get_class($exception);$expandPath=__DIR__.DIRECTORY_SEPARATOR;$counter=0;?><!DOCTYPE html><!-- "' --></script></style></pre></xmp></table>
<html>
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
...
self::highlightFile($ex->getFile(),$ex->getLine(),15,isset($ex->context)?$ex->context:NULL)?><?php endif?>
			...
			<?php if(isset($stack[0]['class'])&&$stack[0]['class']==='HttpPHPUnit\Nette\Diagnostics\Debugger'&&($stack[0]['function']==='_shutdownHandler'||$stack[0]['function']==='_errorHandler'))unset($stack[0])?>
			<?php if($stack):?>
			<div class="panel">
...
htmlSpecialChars($_SERVER['SERVER_SOFTWARE'])?></li><?php endif?>
			<?php if(class_exists('HttpPHPUnit\Nette\Framework')):?><li><?php echo
...
(function(){Nette.Class=function(a){var b=a.constructor||function(){},c,d=Object.prototype.hasOwnProperty;delete a.constructor;if(a.Extends){var f=function(){this.constructor=b};f.prototype=a.Extends.prototype;b.prototype=new f;delete a.Extends}if(a.Static){for(c in a.Static)d.call(a.Static,c)&&(b[c]=a.Static[c]);delete a.Static}for(c in a)d.call(a,c)&&(b.prototype[c]=a[c]);return b};Nette.Q=Nette.Class({Static:{factory:function(a){return new Nette.Q(a)},implement:function(a){var b,c=Nette.Q.implement,
...
b),d=!0);b.position({left:c.clientX+g,top:c.clientY+h});return!1};c.onmouseup=function(f){d&&(a.draggedClass&&b.removeClass(a.draggedClass),a.stop&&a.stop(f||event,b));e.draggable.binded=c.onmousemove=c.onmouseup=null;return!1}}).bind("click",function(a){d&&(a.stopImmediatePropagation(),preventClick=!1)})}})})();
...
htmlSpecialChars(PHP_VERSION." |\n".(isset($_SERVER['SERVER_SOFTWARE'])?$_SERVER['SERVER_SOFTWARE']." |\n":'').(class_exists('HttpPHPUnit\Nette\Framework')?'Nette Framework '.Nette\Framework::VERSION.' ('.substr(Nette\Framework::REVISION,8).')':''))?>">&nbsp;<span>Nette Framework</span></li>
			<?php foreach($panels
...
<?php }return
ob_get_clean();}}}namespace HttpPHPUnit\Nette\Utils{use
HttpPHPUnit\Nette;final
class
SafeStream{const
PROTOCOL='HttpPHPUnitsafe';private$handle;private$tempHandle;private$file;private$tempFile;private$deleteFile;private$writeError=FALSE;static
function
register(){return
...
unlink($path);}}}namespace HttpPHPUnit\Nette\Application{use
HttpPHPUnit\Nette;class
Application
extends
Nette\Object{public
...
restoreRequest($key){return$this->presenter->restoreRequest($key);}}}namespace HttpPHPUnit\Nette\Application\Diagnostics{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Application\Routers;use
HttpPHPUnit\Nette\Application\UI\Presenter;use
HttpPHPUnit\Nette\Diagnostics\Debugger;class
RoutingPanel
extends
Nette\Object
implements
...
instanceof
Routers\Route?$router->getMask():NULL,'request'=>$request,'module'=>rtrim($module,':'));}}}namespace HttpPHPUnit\Nette\Application{use
HttpPHPUnit\Nette;class
AbortException
...
extends
BadRequestException{protected$defaultCode=403;}}namespace HttpPHPUnit\NetteModule{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Application;use
HttpPHPUnit\Nette\Application\Responses;use
HttpPHPUnit\Nette\Http;class
MicroPresenter
extends
Nette\Object
implements
...
Nette\Templating\FileTemplate;$template->setParameters($this->request->getParameters());$template->presenter=$this;$template->context=$context=$this->context;$url=$context->getByType('HttpPHPUnit\Nette\Http\IRequest')->getUrl();$template->baseUrl=rtrim($url->getBaseUrl(),'/');$template->basePath=rtrim($url->getBasePath(),'/');$template->registerHelperLoader('HttpPHPUnit\Nette\Templating\Helpers::loader');$template->setCacheStorage($context->nette->templateCacheStorage);$template->onPrepareFilters[]=function($template)use($latteFactory,$context){$template->registerFilter($latteFactory?$latteFactory():new
...
getRequest(){return$this->request;}}}namespace HttpPHPUnit\Nette\Application{use
HttpPHPUnit\Nette;class
PresenterFactory
implements
...
Nette\Reflection\ClassType($class);$class=$reflection->getName();if(!$reflection->implementsInterface('HttpPHPUnit\Nette\Application\IPresenter')){throw
new
InvalidPresenterException("Cannot load presenter '$name', class '$class' is not HttpPHPUnit\\Nette\\Application\\IPresenter implementor.");}if($reflection->isAbstract()){throw
new
...
formatPresenterFile($presenter){$path='/'.str_replace(':','Module/',$presenter);return$this->baseDir.substr_replace($path,'/presenters',strrpos($path,'/'),0).'Presenter.php';}}}namespace HttpPHPUnit\Nette{use
HttpPHPUnit\Nette;abstract
class
FreezableObject
extends
...
new
InvalidStateException("Cannot modify a frozen object $class.");}}}}namespace HttpPHPUnit\Nette\Application{use
HttpPHPUnit\Nette;final
class
Request
extends
...
hasFlag($flag){return!empty($this->flags[$flag]);}}}namespace HttpPHPUnit\Nette\Application\Responses{use
HttpPHPUnit\Nette;class
FileResponse
extends
Nette\Object
...
instanceof
Nette\Templating\ITemplate){$this->source->render();}else{echo$this->source;}}}}namespace HttpPHPUnit\Nette\Application\Routers{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Application;class
CliRouter
extends
Nette\Object
implements
...
HttpPHPUnit\Nette\Utils\Strings;class
Route
extends
Nette\Object
implements
...
new
Nette\InvalidArgumentException("Style '$style' doesn't exist.");}static::$styles[$style][$key]=$value;}}}namespace HttpPHPUnit\Nette{use
HttpPHPUnit\Nette;class
ArrayList
extends
Object
...
new
OutOfRangeException("Offset invalid or out of range");}array_splice($this->list,(int)$index,1);}}}namespace HttpPHPUnit\Nette\Application\Routers{use
HttpPHPUnit\Nette;class
RouteList
extends
Nette\ArrayList
...
getFlags(){return$this->flags;}}}namespace HttpPHPUnit\Nette\Application\UI{use
HttpPHPUnit\Nette;class
BadSignalException
extends
Nette\Application\BadRequestException{protected$defaultCode=403;}}namespace HttpPHPUnit\Nette\ComponentModel{use
HttpPHPUnit\Nette;abstract
class
Component
extends
...
_isCloning(){return$this->cloning;}}}namespace HttpPHPUnit\Nette\Application\UI{use
HttpPHPUnit\Nette;abstract
class
PresenterComponent
extends
...
__construct(Nette\ComponentModel\IContainer$parent=NULL,$name=NULL){$this->monitor('HttpPHPUnit\Nette\Application\UI\Presenter');parent::__construct($parent,$name);}function
getPresenter($need=TRUE){return$this->lookup('HttpPHPUnit\Nette\Application\UI\Presenter',$need);}function
getUniqueId(){return$this->lookupPath('HttpPHPUnit\Nette\Application\UI\Presenter',TRUE);}protected
function
...
getSnippetId($name=NULL){return'snippet-'.$this->getUniqueId().'-'.$name;}}}namespace HttpPHPUnit\Nette\Forms{use
HttpPHPUnit\Nette;class
Container
extends
Nette\ComponentModel\Container
...
getControls(){return$this->getComponents(TRUE,'HttpPHPUnit\Nette\Forms\IControl');}function
getForm($need=TRUE){return$this->lookup('HttpPHPUnit\Nette\Forms\Form',$need);}function
...
PROTECTION='HttpPHPUnit\Nette\Forms\Controls\HiddenField::validateEqual';const
...
function
getSession(){return
Nette\Environment::getSession();}}}namespace HttpPHPUnit\Nette\Application\UI{use
HttpPHPUnit\Nette;class
Form
extends
Nette\Forms\Form
implements
ISignalReceiver{function
__construct(Nette\ComponentModel\IContainer$parent=NULL,$name=NULL){parent::__construct();$this->monitor('HttpPHPUnit\Nette\Application\UI\Presenter');if($parent!==NULL){$parent->addComponent($this,$name);}}function
getPresenter($need=TRUE){return$this->lookup('HttpPHPUnit\Nette\Application\UI\Presenter',$need);}protected
function
attached($presenter){if($presenter
instanceof
Presenter){$name=$this->lookupPath('HttpPHPUnit\Nette\Application\UI\Presenter');if(!isset($this->getElementPrototype()->id)){$this->getElementPrototype()->id='frm-'.$name;}$this->setAction(new
...
getParameters(){return$this->params;}function
__toString(){try{return$this->component->link($this->destination,$this->params);}catch(\Exception$e){trigger_error("Exception in ".__METHOD__."(): {$e->getMessage()} in {$e->getFile()}:{$e->getLine()}",E_USER_ERROR);}}}class
Multiplier
extends
...
HttpPHPUnit\Nette\Application;use
HttpPHPUnit\Nette\Application\Responses;use
HttpPHPUnit\Nette\Http;use
HttpPHPUnit\Nette\Reflection;abstract
class
Presenter
extends
Control
...
PresenterComponentReflection($forClass):NULL);if($sinces===NULL){$sinces=array();foreach($this->getReflection()->getPersistentParams()as$name=>$meta){$sinces[$name]=$meta['since'];}}$components=$this->getReflection()->getPersistentComponents();$iterator=$this->getComponents(TRUE,'HttpPHPUnit\Nette\Application\UI\IStatePersistent');foreach($iterator
...
function
getHttpRequest(){return$this->context->getByType('HttpPHPUnit\Nette\Http\IRequest');}protected
function
getHttpResponse(){return$this->context->getByType('HttpPHPUnit\Nette\Http\IResponse');}protected
function
getHttpContext(){return$this->context->getByType('HttpPHPUnit\Nette\Http\Context');}function
getApplication(){return$this->context->getByType('HttpPHPUnit\Nette\Application\Application');}function
getSession($namespace=NULL){$handler=$this->context->getByType('HttpPHPUnit\Nette\Http\Session');return$namespace===NULL?$handler:$handler->getSection($namespace);}function
getUser(){return$this->context->getByType('HttpPHPUnit\Nette\Security\User');}}}namespace HttpPHPUnit\Nette\Reflection{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\ObjectMixin;class
ClassType
...
__isset($name){return
ObjectMixin::has($this,$name);}function
__unset($name){ObjectMixin::remove($this,$name);}}}namespace HttpPHPUnit\Nette\Application\UI{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Application\BadRequestException;class
PresenterComponentReflection
extends
...
FALSE;}}return
TRUE;}}}namespace HttpPHPUnit\Nette\Caching{use
HttpPHPUnit\Nette;class
Cache
extends
Nette\Object
...
function()use($cache,$function,$dp){$key=array($function,func_get_args());$data=$cache->load($key);if($data===NULL){$data=$cache->save($key,Nette\Callback::create($function)->invokeArgs($key[1]),$dp);}return$data;};}function
...
new
Nette\InvalidStateException('Output cache has already been saved.');}$this->cache->save($this->key,ob_get_flush(),(array)$dp+(array)$this->dependencies);$this->cache=NULL;}}}namespace HttpPHPUnit\Nette\Caching\Storages{use
HttpPHPUnit\Nette;class
DevNullStorage
extends
Nette\Object
...
HttpPHPUnit\Nette\Caching\Cache;class
FileJournal
extends
Nette\Object
implements
...
parent::getCacheFile(substr_replace($key,trim(strtr($this->hint,'\\/@','.._'),'.').'-',strpos($key,Nette\Caching\Cache::NAMESPACE_SEPARATOR)+1,0)).'.php';}}}namespace HttpPHPUnit\Nette{use
HttpPHPUnit\Nette;class
ArrayHash
extends\stdClass
...
new
Nette\InvalidStateException('Configurator & SystemContainer has already been created automatically by HttpPHPUnit\Nette\Environment at '.self::$createdAt);}self::$context=$context;}static
function
...
new
MemberAccessException("Call to undefined static method HttpPHPUnit\\Nette\\Environment::$name().");}}static
function
getHttpRequest(){return
self::getContext()->getByType('HttpPHPUnit\Nette\Http\IRequest');}static
function
getHttpContext(){return
self::getContext()->getByType('HttpPHPUnit\Nette\Http\Context');}static
function
getHttpResponse(){return
self::getContext()->getByType('HttpPHPUnit\Nette\Http\IResponse');}static
function
getApplication(){return
self::getContext()->getByType('HttpPHPUnit\Nette\Application\Application');}static
function
getUser(){return
self::getContext()->getByType('HttpPHPUnit\Nette\Security\User');}static
function
getRobotLoader(){return
self::getContext()->getByType('HttpPHPUnit\Nette\Loaders\RobotLoader');}static
function
...
new
Nette\InvalidStateException('HttpPHPUnit\Nette\Config\Configurator has already been created automatically by HttpPHPUnit\Nette\Environment at '.self::$createdAt);}$configurator=new
Nette\Config\Configurator;$configurator->setDebugMode(!self::isProduction())->setTempDirectory(defined('TEMP_DIR')?TEMP_DIR:'');if($file){$configurator->addConfig($file,$section);}self::$context=$configurator->createContainer();self::$createdAt='?';foreach(debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],__DIR__.DIRECTORY_SEPARATOR)!==0){self::$createdAt="$row[file]:$row[line]";break;}}return
...
class
Framework{const
NAME='Nette Framework',VERSION='2.0.6',REVISION='6a33aa6 released on 2012-10-01';public
...
static
function
hasProperty($class,$name){$prop=&self::$props[$class][$name];if($prop===NULL){$prop=FALSE;try{$rp=new\ReflectionProperty($class,$name);if($name===$rp->getName()&&$rp->isPublic()&&!$rp->isStatic()){$prop=preg_match('#^on[A-Z]#',$name)?'event':TRUE;}}catch(\ReflectionException$e){}}return$prop;}}}namespace HttpPHPUnit\Nette\ComponentModel{use
HttpPHPUnit\Nette;class
...
count(){return
iterator_count($this);}}}namespace HttpPHPUnit\Nette\Config\Adapters{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Config\Helpers;class
IniAdapter
extends
Nette\Object
implements
...
as$name=>$secData){if(!is_array($secData)){$output=array();self::build($data,$output,'');break;}if($parent=Helpers::takeParent($secData)){$output[]="[$name ".self::INHERITING_SEPARATOR." $parent]";}else{$output[]="[$name]";}self::build($secData,$output,'');$output[]='';}return"; generated by HttpPHPUnit\Nette\n\n".implode(PHP_EOL,$output);}private
static
function
...
HttpPHPUnit\Nette\Utils\Neon;class
NeonAdapter
extends
Nette\Object
implements
...
as$name=>$secData){if($parent=Helpers::takeParent($secData)){$name.=' '.self::INHERITING_SEPARATOR.' '.$parent;}$tmp[$name]=$secData;}return"# generated by HttpPHPUnit\Nette\n\n".Neon::encode($tmp,Neon::BLOCK);}}class
PhpAdapter
extends
Nette\Object
implements
...
dump(array$data){return"<?php // generated by Nette \nreturn ".Nette\Utils\PhpGenerator\Helpers::dump($data).';';}}}namespace HttpPHPUnit\Nette\Config{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Utils\Validators;class
Compiler
extends
...
as$name=>$extension){$this->container->addDefinition($name)->setClass('HttpPHPUnit\Nette\DI\NestedAccessor',array('@container',$name))->setAutowired(FALSE);if(isset($this->config[$name])){$this->parseServices($this->container,$this->config[$name],$name);}}foreach($this->container->getDefinitions()as$name=>$def){$factory=$name.'Factory';if(!$def->shared&&!$def->internal&&!$this->container->hasDefinition($factory)){$this->container->addDefinition($factory)->setClass('HttpPHPUnit\Nette\Callback',array('@container',Nette\DI\Container::getMethodName($name,FALSE)))->setAutowired(FALSE)->tags=$def->tags;}}}function
...
as$extension){$extension->afterCompile($class);}$defs=$this->container->getDefinitions();ksort($defs);$list=array_keys($defs);foreach(array_reverse($defs,TRUE)as$name=>$def){if($def->class==='HttpPHPUnit\Nette\DI\NestedAccessor'&&($found=preg_grep('#^'.$name.'\.#i',$list))){$list=array_diff($list,$found);$def->class=$className.'_'.preg_replace('#\W+#','_',$name);$class->documents=preg_replace("#\S+(?= \\$$name$)#",$def->class,$class->documents);$classes[]=$accessor=new
...
HttpPHPUnit\Nette\DI\ContainerBuilder;abstract
class
CompilerExtension
extends
...
array('appDir'=>isset($trace[1]['file'])?dirname($trace[1]['file']):NULL,'wwwDir'=>isset($_SERVER['SCRIPT_FILENAME'])?dirname($_SERVER['SCRIPT_FILENAME']):NULL,'debugMode'=>$debugMode,'productionMode'=>!$debugMode,'environment'=>$debugMode?self::DEVELOPMENT:self::PRODUCTION,'consoleMode'=>PHP_SAPI==='cli','container'=>array('class'=>'SystemContainer','parent'=>'HttpPHPUnit\Nette\DI\Container'));}function
...
function
detectProductionMode($list=NULL){return!static::detectDebugMode($list);}}}namespace HttpPHPUnit\Nette\Config\Extensions{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\DI\ContainerBuilder;class
ConstantsExtension
extends
...
NetteExtension
extends
Nette\Config\CompilerExtension{public$defaults=array('xhtml'=>TRUE,'session'=>array('iAmUsingBadHost'=>NULL,'autoStart'=>'smart','expiration'=>NULL),'application'=>array('debugger'=>TRUE,'errorPresenter'=>NULL,'catchExceptions'=>'%productionMode%'),'routing'=>array('debugger'=>TRUE,'routes'=>array()),'security'=>array('debugger'=>TRUE,'frames'=>'SAMEORIGIN','users'=>array(),'roles'=>array(),'resources'=>array()),'mailer'=>array('smtp'=>FALSE),'database'=>array(),'forms'=>array('messages'=>array()),'container'=>array('debugger'=>FALSE),'debugger'=>array('email'=>NULL,'editor'=>NULL,'browser'=>NULL,'strictMode'=>NULL,'bar'=>array(),'blueScreen'=>array()));public$databaseDefaults=array('dsn'=>NULL,'user'=>NULL,'password'=>NULL,'options'=>NULL,'debugger'=>TRUE,'explain'=>TRUE,'reflection'=>'HttpPHPUnit\Nette\Database\Reflection\DiscoveredReflection');function
loadConfiguration(){$container=$this->getContainerBuilder();$config=$this->getConfig($this->defaults);$container->addDefinition($this->prefix('cacheJournal'))->setClass('HttpPHPUnit\Nette\Caching\Storages\FileJournal',array('%tempDir%'));$container->addDefinition('cacheStorage')->setClass('HttpPHPUnit\Nette\Caching\Storages\FileStorage',array('%tempDir%/cache'));$container->addDefinition($this->prefix('templateCacheStorage'))->setClass('HttpPHPUnit\Nette\Caching\Storages\PhpFileStorage',array('%tempDir%/cache'))->setAutowired(FALSE);$container->addDefinition($this->prefix('cache'))->setClass('HttpPHPUnit\Nette\Caching\Cache',array(1=>'%namespace%'))->setParameters(array('namespace'=>NULL));$container->addDefinition($this->prefix('httpRequestFactory'))->setClass('HttpPHPUnit\Nette\Http\RequestFactory')->addSetup('setEncoding',array('UTF-8'))->setInternal(TRUE);$container->addDefinition('httpRequest')->setClass('HttpPHPUnit\Nette\Http\Request')->setFactory('@Nette\Http\RequestFactory::createHttpRequest');$container->addDefinition('httpResponse')->setClass('HttpPHPUnit\Nette\Http\Response');$container->addDefinition($this->prefix('httpContext'))->setClass('HttpPHPUnit\Nette\Http\Context');$session=$container->addDefinition('session')->setClass('HttpPHPUnit\Nette\Http\Session');if(isset($config['session']['expiration'])){$session->addSetup('setExpiration',array($config['session']['expiration']));}if(isset($config['session']['iAmUsingBadHost'])){$session->addSetup('HttpPHPUnit\Nette\Framework::$iAmUsingBadHost = ?;',array((bool)$config['session']['iAmUsingBadHost']));}unset($config['session']['expiration'],$config['session']['autoStart'],$config['session']['iAmUsingBadHost']);if(!empty($config['session'])){$session->addSetup('setOptions',array($config['session']));}$container->addDefinition($this->prefix('userStorage'))->setClass('HttpPHPUnit\Nette\Http\UserStorage');$user=$container->addDefinition('user')->setClass('HttpPHPUnit\Nette\Security\User');if(!$container->parameters['productionMode']&&$config['security']['debugger']){$user->addSetup('HttpPHPUnit\Nette\Diagnostics\Debugger::$bar->addPanel(?)',array(new
Nette\DI\Statement('HttpPHPUnit\Nette\Security\Diagnostics\UserPanel')));}if($config['security']['users']){$container->addDefinition($this->prefix('authenticator'))->setClass('HttpPHPUnit\Nette\Security\SimpleAuthenticator',array($config['security']['users']));}if($config['security']['roles']||$config['security']['resources']){$authorizator=$container->addDefinition($this->prefix('authorizator'))->setClass('HttpPHPUnit\Nette\Security\Permission');foreach($config['security']['roles']as$role=>$parents){$authorizator->addSetup('addRole',array($role,$parents));}foreach($config['security']['resources']as$resource=>$parents){$authorizator->addSetup('addResource',array($resource,$parents));}}$application=$container->addDefinition('application')->setClass('HttpPHPUnit\Nette\Application\Application')->addSetup('$catchExceptions',$config['application']['catchExceptions'])->addSetup('$errorPresenter',$config['application']['errorPresenter']);if($config['application']['debugger']){$application->addSetup('HttpPHPUnit\Nette\Application\Diagnostics\RoutingPanel::initializePanel');}$container->addDefinition($this->prefix('presenterFactory'))->setClass('HttpPHPUnit\Nette\Application\PresenterFactory',array(isset($container->parameters['appDir'])?$container->parameters['appDir']:NULL));$router=$container->addDefinition('router')->setClass('HttpPHPUnit\Nette\Application\Routers\RouteList');foreach($config['routing']['routes']as$mask=>$action){$router->addSetup('$service[] = new HttpPHPUnit\Nette\Application\Routers\Route(?, ?);',array($mask,$action));}if(!$container->parameters['productionMode']&&$config['routing']['debugger']){$application->addSetup('HttpPHPUnit\Nette\Diagnostics\Debugger::$bar->addPanel(?)',array(new
Nette\DI\Statement('HttpPHPUnit\Nette\Application\Diagnostics\RoutingPanel')));}if(empty($config['mailer']['smtp'])){$container->addDefinition($this->prefix('mailer'))->setClass('HttpPHPUnit\Nette\Mail\SendmailMailer');}else{$container->addDefinition($this->prefix('mailer'))->setClass('HttpPHPUnit\Nette\Mail\SmtpMailer',array($config['mailer']));}$container->addDefinition($this->prefix('mail'))->setClass('HttpPHPUnit\Nette\Mail\Message')->addSetup('setMailer')->setShared(FALSE);$container->addDefinition($this->prefix('basicForm'))->setClass('HttpPHPUnit\Nette\Forms\Form')->setShared(FALSE);$latte=$container->addDefinition($this->prefix('latte'))->setClass('HttpPHPUnit\Nette\Latte\Engine')->setShared(FALSE);if(empty($config['xhtml'])){$latte->addSetup('$service->getCompiler()->defaultContentType = ?',Nette\Latte\Compiler::CONTENT_HTML);}$container->addDefinition($this->prefix('template'))->setClass('HttpPHPUnit\Nette\Templating\FileTemplate')->addSetup('registerFilter',array($latte))->addSetup('registerHelperLoader',array('HttpPHPUnit\Nette\Templating\Helpers::loader'))->setShared(FALSE);$container->addDefinition($this->prefix('database'))->setClass('HttpPHPUnit\Nette\DI\NestedAccessor',array('@container',$this->prefix('database')));if(isset($config['database']['dsn'])){$config['database']=array('default'=>$config['database']);}$autowired=TRUE;foreach((array)$config['database']as$name=>$info){if(!is_array($info)){continue;}$info+=$this->databaseDefaults+array('autowired'=>$autowired);$autowired=FALSE;foreach((array)$info['options']as$key=>$value){unset($info['options'][$key]);$info['options'][constant($key)]=$value;}$connection=$container->addDefinition($this->prefix("database.$name"))->setClass('HttpPHPUnit\Nette\Database\Connection',array($info['dsn'],$info['user'],$info['password'],$info['options']))->setAutowired($info['autowired'])->addSetup('setCacheStorage')->addSetup('HttpPHPUnit\Nette\Diagnostics\Debugger::$blueScreen->addPanel(?)',array('HttpPHPUnit\Nette\Database\Diagnostics\ConnectionPanel::renderException'));if($info['reflection']){$connection->addSetup('setDatabaseReflection',is_string($info['reflection'])?array(new
Nette\DI\Statement(preg_match('#^[a-z]+$#',$info['reflection'])?'HttpPHPUnit\Nette\Database\Reflection\\'.ucfirst($info['reflection']).'Reflection':$info['reflection'])):Nette\Config\Compiler::filterArguments(array($info['reflection'])));}if(!$container->parameters['productionMode']&&$info['debugger']){$panel=$container->addDefinition($this->prefix("database.{$name}ConnectionPanel"))->setClass('HttpPHPUnit\Nette\Database\Diagnostics\ConnectionPanel')->setAutowired(FALSE)->addSetup('$explain',!empty($info['explain']))->addSetup('HttpPHPUnit\Nette\Diagnostics\Debugger::$bar->addPanel(?)',array('@self'));$connection->addSetup('$service->onQuery[] = ?',array(array($panel,'logQuery')));}}}function
afterCompile(Nette\Utils\PhpGenerator\ClassType$class){$initialize=$class->methods['initialize'];$container=$this->getContainerBuilder();$config=$this->getConfig($this->defaults);foreach(array('email','editor','browser','strictMode','maxLen','maxDepth')as$key){if(isset($config['debugger'][$key])){$initialize->addBody('HttpPHPUnit\Nette\Diagnostics\Debugger::$? = ?;',array($key,$config['debugger'][$key]));}}if(!$container->parameters['productionMode']){if($config['container']['debugger']){$config['debugger']['bar'][]='HttpPHPUnit\Nette\DI\Diagnostics\ContainerPanel';}foreach((array)$config['debugger']['bar']as$item){$initialize->addBody($container->formatPhp('HttpPHPUnit\Nette\Diagnostics\Debugger::$bar->addPanel(?);',Nette\Config\Compiler::filterArguments(array(is_string($item)?new
Nette\DI\Statement($item):$item))));}foreach((array)$config['debugger']['blueScreen']as$item){$initialize->addBody($container->formatPhp('HttpPHPUnit\Nette\Diagnostics\Debugger::$blueScreen->addPanel(?);',Nette\Config\Compiler::filterArguments(array($item))));}}if(!empty($container->parameters['tempDir'])){$initialize->addBody($this->checkTempDir($container->expand('%tempDir%/cache')));}foreach((array)$config['forms']['messages']as$name=>$text){$initialize->addBody('HttpPHPUnit\Nette\Forms\Rules::$defaultMessages[Nette\Forms\Form::?] = ?;',array($name,$text));}if($config['session']['autoStart']==='smart'){$initialize->addBody('$this->session->exists() && $this->session->start();');}elseif($config['session']['autoStart']){$initialize->addBody('$this->session->start();');}if(empty($config['xhtml'])){$initialize->addBody('HttpPHPUnit\Nette\Utils\Html::$xhtml = ?;',array((bool)$config['xhtml']));}if(isset($config['security']['frames'])&&$config['security']['frames']!==TRUE){$frames=$config['security']['frames'];if($frames===FALSE){$frames='DENY';}elseif(preg_match('#^https?:#',$frames)){$frames="ALLOW-FROM $frames";}$initialize->addBody('header(?);',array("X-Frame-Options: $frames"));}foreach($container->findByTag('run')as$name=>$on){if($on){$initialize->addBody('$this->getService(?);',array($name));}}}private
function
checkTempDir($dir){$uniq=uniqid('_',TRUE);if(!@mkdir("$dir/$uniq",0777)){throw
new
Nette\InvalidStateException("Unable to write to directory '$dir'. Make this directory writable.");}$useDirs=@file_put_contents("$dir/$uniq/_",'')!==FALSE;@unlink("$dir/$uniq/_");@rmdir("$dir/$uniq");return'HttpPHPUnit\Nette\Caching\Storages\FileStorage::$useDirectories = '.($useDirs?'TRUE':'FALSE').";\n";}}class
PhpExtension
extends
Nette\Config\CompilerExtension{function
...
new
Nette\NotSupportedException('Required function ini_set() is disabled.');}}}}}namespace HttpPHPUnit\Nette\Config{use
HttpPHPUnit\Nette;class
Helpers{const
...
extends
Nette\Object{const
INCLUDES_KEY='includes';private$adapters=array('php'=>'HttpPHPUnit\Nette\Config\Adapters\PhpAdapter','ini'=>'HttpPHPUnit\Nette\Config\Adapters\IniAdapter','neon'=>'HttpPHPUnit\Nette\Config\Adapters\NeonAdapter');private$dependencies=array();function
...
function
getSection(array$data,$key,$file){Validators::assertField($data,$key,'array|null',"section '%' in file '$file'");$item=$data[$key];if($parent=Helpers::takeParent($item)){$item=Helpers::merge($item,$this->getSection($data,$parent,$file));}return$item;}}}namespace HttpPHPUnit\Nette\Database{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\ObjectMixin;use
...
__isset($name){return
ObjectMixin::has($this,$name);}function
__unset($name){ObjectMixin::remove($this,$name);}}}}namespace HttpPHPUnit\Nette\Database\Diagnostics{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Database\Helpers;use
HttpPHPUnit\Nette\Diagnostics\Debugger;class
ConnectionPanel
extends
Nette\Object
...
logQuery(Nette\Database\Statement$result,array$params=NULL){if($this->disabled){return;}$source=NULL;foreach(debug_backtrace(FALSE)as$row){if(isset($row['file'])&&is_file($row['file'])&&strpos($row['file'],__DIR__.DIRECTORY_SEPARATOR)!==0){if(isset($row['function'])&&strpos($row['function'],'call_user_func')===0)continue;if(isset($row['class'])&&is_subclass_of($row['class'],'\\Nette\\Database\\Connection'))continue;$source=array($row['file'],(int)$row['line']);break;}}$this->totalTime+=$result->getTime();$this->queries[]=array($result->queryString,$params,$result->getTime(),$result->rowCount(),$result->getConnection(),$source);}static
function
...
empty($this->queries)?'':'<style> #nette-debug td.nette-DbConnectionPanel-sql { background: white !important }
			...
			</div>';}}}namespace HttpPHPUnit\Nette\Database\Drivers{use
HttpPHPUnit\Nette;class
MsSqlDriver
extends
Nette\Object
...
getForeignKeys($table){throw
new
NotSupportedException;}}}namespace HttpPHPUnit\Nette\Database{use
HttpPHPUnit\Nette;class
Helpers{public
...
new
Nette\FileNotFoundException("Cannot open file '$file'.");}$count=0;$sql='';while(!feof($handle)){$s=fgets($handle);$sql.=$s;if(substr(rtrim($s),-1)===';'){$connection->exec($sql);$sql='';$count++;}}if(trim($sql)!==''){$connection->exec($sql);$count++;}fclose($handle);return$count;}}}namespace HttpPHPUnit\Nette\Database\Reflection{use
HttpPHPUnit\Nette;class
...
Nette\Object
implements
Nette\Database\IReflection{protected$cache;protected$cacheStorage;protected$connection;protected$structure=array();function
...
getHasManyReference($table,$key,$refresh=TRUE){$reference=&$this->structure['hasMany'][strtolower($table)];if(!empty($reference)){$candidates=$columnCandidates=array();foreach($reference
as$targetPair){list($targetColumn,$targetTable)=$targetPair;if(stripos($targetTable,$key)===FALSE)continue;$candidates[]=array($targetTable,$targetColumn);if(stripos($targetColumn,$table)!==FALSE){$columnCandidates[]=$candidate=array($targetTable,$targetColumn);if(strtolower($targetTable)===strtolower($key))return$candidate;}}if(count($columnCandidates)===1){return
reset($columnCandidates);}elseif(count($candidates)===1){return
reset($candidates);}foreach($candidates
as$candidate){list($targetTable,$targetColumn)=$candidate;if(strtolower($targetTable)===strtolower($key))return$candidate;}if(!$refresh&&!empty($candidates)){throw
new\PDOException('Ambiguous joining column in related call.');}}if(!$refresh){throw
...
getBelongsToReference($table,$key,$refresh=TRUE){$reference=&$this->structure['belongsTo'][strtolower($table)];if(!empty($reference)){foreach($reference
as$column=>$targetTable){if(stripos($column,$key)!==FALSE){return
...
strlen($a)-strlen($b);});}}}}namespace HttpPHPUnit\Nette\Database{use
HttpPHPUnit\Nette;class
Row
extends
...
PDO;use
HttpPHPUnit\Nette\ObjectMixin;if(class_exists('PDO')){class
Statement
extends\PDOStatement{private$connection;private$time;private$types;protected
function
__construct(Connection$connection){$this->connection=$connection;$this->setFetchMode(PDO::FETCH_CLASS,'HttpPHPUnit\Nette\Database\Row',array($this));}function
...
__isset($name){return
ObjectMixin::has($this,$name);}function
__unset($name){ObjectMixin::remove($this,$name);}}}}namespace HttpPHPUnit\Nette\Database\Table{use
HttpPHPUnit\Nette;class
ActiveRow
extends
Nette\Object
...
FALSE;}}use
HttpPHPUnit\Nette\Database\ISupplementalDriver;use
PDO;class
Selection
extends
Nette\Object
...
get($key){$clone=clone$this;$clone->where($this->primary,$key);return$clone->fetch();}function
...
find($key){return$this->where($this->primary,$key);}function
...
offsetUnset($key){$this->execute();unset($this->data[$key]);}}class
GroupedSelection
extends
...
order($columns){if(!$this->sqlBuilder->getOrder()){$this->sqlBuilder->addOrder("$this->name.$this->column".(preg_match('~\\bDESC$~i',$columns)?' DESC':''));}return
...
SqlBuilder
extends
Nette\Object{protected$selection;protected$connection;protected$delimitedTable;protected$select=array();protected$where=array();protected$conditions=array();protected$parameters=array();protected$order=array();protected$limit=NULL;protected$offset=NULL;protected$group='';protected$having='';function
__construct(Selection$selection){$this->selection=$selection;$this->connection=$selection->getConnection();$this->delimitedTable=$this->tryDelimite($selection->getName());}function
...
instanceof
Selection){$clone=clone$parameters;if(!$clone->getSqlBuilder()->select){$clone->select($clone->primary);}if($this->connection->getAttribute(PDO::ATTR_DRIVER_NAME)!=='mysql'){$condition.=' IN ('.$clone->getSql().')';}else{$in=array();foreach($clone
...
buildSelectQuery(){$join=$this->buildJoins(implode(',',$this->conditions),TRUE);$join+=$this->buildJoins(implode(',',$this->select).",{$this->group},{$this->having},".implode(',',$this->order));$prefix=$join?"{$this->delimitedTable}.":'';if($this->select){$cols=$this->tryDelimite($this->removeExtraTables(implode(', ',$this->select)));}elseif($prevAccessed=$this->selection->getPreviousAccessed()){$cols=array_map(array($this->connection->getSupplementalDriver(),'delimite'),array_keys(array_filter($prevAccessed)));$cols=$prefix.implode(', '.$prefix,$cols);}elseif($this->group&&!$this->connection->getSupplementalDriver()->isSupported(ISupplementalDriver::SUPPORT_SELECT_UNGROUPED_COLUMNS)){$cols=$this->tryDelimite($this->removeExtraTables($this->group));}else{$cols=$prefix.'*';}return"SELECT{$this->buildTopClause()} {$cols} FROM {$this->delimitedTable}".implode($join).$this->buildConditions();}function
getParameters(){return$this->parameters;}protected
function
buildJoins($val,$inner=FALSE){$driver=$this->selection->getConnection()->getSupplementalDriver();$reflection=$this->selection->getConnection()->getDatabaseReflection();$joins=array();preg_match_all('~\\b([a-z][\\w.:]*[.:])([a-z]\\w*|\*)(\\s+IS\\b|\\s*<=>)?~i',$val,$matches);foreach($matches[1]as$names){$parent=$this->selection->getName();if($names!=="$parent."){preg_match_all('~\\b([a-z][\\w]*|\*)([.:])~i',$names,$matches,PREG_SET_ORDER);foreach($matches
...
preg_replace('~(?:\\b[a-z_][a-z0-9_.:]*[.:])?([a-z_][a-z0-9_]*)[.:]([a-z_*])~i','\\1.\\2',$expression);}}}namespace HttpPHPUnit\Nette\DI{use
HttpPHPUnit\Nette;class
Container
extends
...
HttpPHPUnit\Nette\Utils\Validators;use
HttpPHPUnit\Nette\Utils\Strings;use
HttpPHPUnit\Nette\Utils\PhpGenerator\Helpers as PhpHelpers;use
HttpPHPUnit\Nette\Utils\PhpGenerator\PhpLiteral;class
ContainerBuilder
extends
Nette\Object{const
...
generateClass($parentClass='HttpPHPUnit\Nette\DI\Container'){unset($this->definitions[self::THIS_CONTAINER]);$this->addDefinition(self::THIS_CONTAINER)->setClass($parentClass);$this->prepareClassList();$class=new
...
new
ServiceCreationException("Reference to missing service '$service'.");}return$service;}}}namespace HttpPHPUnit\Nette\DI\Diagnostics{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\DI\Container;use
HttpPHPUnit\Nette\Diagnostics\Helpers;class
ContainerPanel
extends
Nette\Object
implements
...
ob_get_clean();}private
function
getContainerProperty($name){$prop=Nette\Reflection\ClassType::from('HttpPHPUnit\Nette\DI\Container')->getProperty($name);$prop->setAccessible(TRUE);return$prop->getValue($this->container);}}}namespace HttpPHPUnit\Nette\DI{use
HttpPHPUnit\Nette;class
...
__construct($entity,array$arguments=array()){$this->entity=$entity;$this->arguments=$arguments;}}}namespace HttpPHPUnit\Nette\Forms{use
HttpPHPUnit\Nette;class
ControlGroup
extends
...
function
getOptions(){return$this->options;}}}namespace HttpPHPUnit\Nette\Forms\Controls{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Forms\IControl;use
HttpPHPUnit\Nette\Utils\Html;use
HttpPHPUnit\Nette\Forms\Form;use
HttpPHPUnit\Nette\Forms\Rule;abstract
class
BaseControl
extends
...
__construct($caption=NULL){$this->monitor('HttpPHPUnit\Nette\Forms\Form');parent::__construct();$this->control=Html::el('input');$this->label=Html::el('label');$this->caption=$caption;$this->rules=new
...
getForm($need=TRUE){return$this->lookup('HttpPHPUnit\Nette\Forms\Form',$need);}function
getHtmlName(){if($this->htmlName===NULL){$name=str_replace(self::NAME_SEPARATOR,'][',$this->lookupPath('HttpPHPUnit\Nette\Forms\Form'),$count);if($count){$name=substr_replace($name,'',strpos($name,']'),1).']';}if(is_numeric($name)||in_array($name,array('attributes','children','elements','focus','length','reset','style','submit','onsubmit'))){$name.='_';}$this->htmlName=$name;}return$this->htmlName;}function
setHtmlId($id){$this->htmlId=$id;return$this;}function
getHtmlId(){if($this->htmlId===FALSE){return
NULL;}elseif($this->htmlId===NULL){$this->htmlId=sprintf(self::$idMask,$this->getForm()->getName(),$this->lookupPath('HttpPHPUnit\Nette\Forms\Form'));}return$this->htmlId;}function
...
HttpPHPUnit\Nette\Utils\Strings;use
HttpPHPUnit\Nette\Utils\Validators;abstract
class
TextBase
extends
...
instanceof
Http\FileUpload&&$file->isImage();}}}namespace HttpPHPUnit\Nette\Forms\Rendering{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Utils\Html;class
DefaultFormRenderer
extends
Nette\Object
implements
...
instanceof
Nette\Forms\Controls\HiddenField&&!$control->getOption('rendered')){$s.=(string)$control->getControl();}}if(iterator_count($this->form->getComponents(TRUE,'HttpPHPUnit\Nette\Forms\Controls\TextInput'))<2){$s.='<!--[if IE]><input type=IEbug disabled style="display:none"><![endif]-->';}if($s){$s=$this->getWrapper('hidden container')->setHtml($s)."\n";}return$s.$this->form->getElementPrototype()->endTag()."\n";}function
...
function
getValue($name){$name=explode(' ',$name);$data=&$this->wrappers[$name[0]][$name[1]];return$data;}}}namespace HttpPHPUnit\Nette\Forms{use
HttpPHPUnit\Nette;final
class
Rule
extends
...
getSection($section,$class='HttpPHPUnit\Nette\Http\SessionSection'){return
...
HttpPHPUnit\Nette\Security\IIdentity;class
UserStorage
extends
Nette\Object
implements
...
count(){return
iterator_count($this);}}}namespace HttpPHPUnit\Nette\Latte{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Utils\Strings;class
Compiler
extends
...
getCompiler(){return$this->compiler;}}}namespace HttpPHPUnit\Nette\Templating{use
HttpPHPUnit\Nette;class
FilterException
extends
...
setSourceFile($file){$this->sourceFile=(string)$file;$this->message=rtrim($this->message,'.')." in ".str_replace(dirname(dirname($file)),'...',$file).($this->sourceLine?":$this->sourceLine":'');}}}namespace HttpPHPUnit\Nette\Latte{use
HttpPHPUnit\Nette;class
CompileException
extends
Nette\Templating\FilterException{}class_alias('HttpPHPUnit\Nette\Latte\CompileException','HttpPHPUnit\Nette\Latte\ParseException');class
HtmlNode
extends
...
setArgs($args){$this->args=(string)$args;$this->tokenizer->tokenize($this->args);}}}namespace HttpPHPUnit\Nette\Latte\Macros{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Latte;class
CacheMacro
extends
Nette\Object
implements
...
array('HttpPHPUnit\Nette\Latte\Macros\CacheMacro::initRuntime($template, $_g);');}}function
...
HttpPHPUnit\Nette\Latte\MacroNode;class
MacroSet
extends
Nette\Object
implements
...
HttpPHPUnit\Nette\Latte\CompileException;use
HttpPHPUnit\Nette\Latte\PhpWriter;class
CoreMacros
extends
MacroSet{static
function
...
finalize(){return
array('list($_l, $_g) = HttpPHPUnit\Nette\Latte\Macros\CoreMacros::initRuntime($template, '.var_export($this->getCompiler()->getTemplateId(),TRUE).')');}function
...
macroInclude(MacroNode$node,PhpWriter$writer){$code=$writer->write('HttpPHPUnit\Nette\Latte\Macros\CoreMacros::includeTemplate(%node.word, %node.array? + $template->getParameters(), $_l->templates[%var])',$this->getCompiler()->getTemplateId());if($node->modifiers){return$writer->write('echo %modify(%raw->__toString(TRUE))',$code);}else{return$code.'->render()';}}function
...
macroAttr(MacroNode$node,PhpWriter$writer){return$writer->write('echo HttpPHPUnit\Nette\Utils\Html::el(NULL, %node.array)->attributes()');}function
macroOldAttr(MacroNode$node){return
Nette\Utils\Strings::replace($node->args.' ','#\)\s+#',')->');}function
macroDump(MacroNode$node,PhpWriter$writer){$args=$writer->formatArgs();return'HttpPHPUnit\Nette\Diagnostics\Debugger::barDump('.($node->args?"array(".$writer->write('%var',$args)." => $args)":'get_defined_vars()').', "Template " . str_replace(dirname(dirname($template->getFile())), "\xE2\x80\xA6", $template->getFile()))';}function
...
new
Nette\NotSupportedException('Macro {include "filename"} is supported only with HttpPHPUnit\Nette\Templating\IFileTemplate.');}$tpl->setParameters($params);return$tpl;}static
function
...
function
install(Latte\Compiler$compiler){$me=new
static($compiler);$me->addMacro('form','HttpPHPUnit\Nette\Latte\Macros\FormMacros::renderFormBegin($form = $_form = (is_object(%node.word) ? %node.word : $_control[%node.word]), %node.array)','HttpPHPUnit\Nette\Latte\Macros\FormMacros::renderFormEnd($_form)');$me->addMacro('label',array($me,'macroLabel'),'?></label><?php');$me->addMacro('input','echo $_form[%node.word]->getControl()->addAttributes(%node.array)',NULL,array($me,'macroAttrInput'));$me->addMacro('formContainer','$_formStack[] = $_form; $formContainer = $_form = $_form[%node.word]','$_form = array_pop($_formStack)');}function
...
function
renderFormEnd(Form$form){$s='';if(strcasecmp($form->getMethod(),'get')===0){$url=explode('?',$form->getElementPrototype()->action,2);if(isset($url[1])){foreach(preg_split('#[;&]#',$url[1])as$param){$parts=explode('=',$param,2);$name=urldecode($parts[0]);if(!isset($form[$name])){$s.=Nette\Utils\Html::el('input',array('type'=>'hidden','name'=>$name,'value'=>urldecode($parts[1])));}}}}foreach($form->getComponents(TRUE,'HttpPHPUnit\Nette\Forms\Controls\HiddenField')as$control){if(!$control->getOption('rendered')){$s.=$control->getControl();}}if(iterator_count($form->getComponents(TRUE,'HttpPHPUnit\Nette\Forms\Controls\TextInput'))<2){$s.='<!--[if IE]><input type=IEbug disabled style="display:none"><![endif]-->';}echo($s?"<div>$s</div>\n":'').$form->getElementPrototype()->endTag()."\n";}}class
UIMacros
extends
...
as$name=>$code){$func='_lb'.substr(md5($this->getCompiler()->getTemplateId().$name),0,10).'_'.preg_replace('#[^a-z0-9_]#i','_',$name);$snippet=$name[0]==='_';$prolog[]="//\n// block $name\n//\n"."if (!function_exists(\$_l->blocks[".var_export($name,TRUE)."][] = '$func')) { "."function $func(\$_l, \$_args) { ".(PHP_VERSION_ID>50208?'extract($_args)':'foreach ($_args as $__k => $__v) $$__k = $__v').($snippet?'; $_control->validateControl('.var_export(substr($name,1),TRUE).')':'')."\n?>$code<?php\n}}";}$prolog[]="//\n// end of blocks\n//";}if($this->namedBlocks||$this->extends){$prolog[]="// template extending and snippets support";$prolog[]='$_l->extends = '.($this->extends?$this->extends:'empty($template->_extended) && isset($_control) && $_control instanceof HttpPHPUnit\Nette\Application\UI\Presenter ? $_control->findLayoutTemplateFile() : NULL').'; $template->_extended = $_extended = TRUE;';$prolog[]='
if ($_l->extends) {
	'.($this->namedBlocks?'ob_start();':'return HttpPHPUnit\Nette\Latte\Macros\CoreMacros::includeTemplate($_l->extends, get_defined_vars(), $template)->render();').'

} elseif (!empty($_control->snippetMode)) {
	return HttpPHPUnit\Nette\Latte\Macros\UIMacros::renderSnippets($_control, $_l, get_defined_vars());
}';}else{$prolog[]='
// snippets support
...
new
CompileException("Missing control name in {control}");}$pair=explode(':',$pair,2);$name=$writer->formatWord($pair[0]);$method=isset($pair[1])?ucfirst($pair[1]):'';$method=Strings::match($method,'#^\w*$#')?"render$method":"{\"render$method\"}";$param=$writer->formatArray();if(!Strings::contains($node->args,'=>')){$param=substr($param,6,-1);}return($name[0]==='$'?"if (is_object($name)) \$_ctrl = $name; else ":'').'$_ctrl = $_control->getComponent('.$name.'); '.'if ($_ctrl instanceof HttpPHPUnit\Nette\Application\UI\IRenderable) $_ctrl->validateControl(); '."\$_ctrl->$method($param)";}function
...
instanceof
Nette\ComponentModel\IContainer){$queue[]=$child;}}}while($queue);}}}}namespace HttpPHPUnit\Nette\Utils{use
HttpPHPUnit\Nette;class
Tokenizer
extends
...
TokenizerException
extends\Exception{}}namespace HttpPHPUnit\Nette\Latte{use
HttpPHPUnit\Nette;class
MacroTokenizer
extends
...
Compiler::CONTEXT_DOUBLE_QUOTED:if($context[1]===Compiler::CONTENT_JS){$s="HttpPHPUnit\\Nette\\Templating\\Helpers::escapeJs($s)";}elseif($context[1]===Compiler::CONTENT_CSS){$s="HttpPHPUnit\\Nette\\Templating\\Helpers::escapeCss($s)";}$quote=$context[0]===Compiler::CONTEXT_DOUBLE_QUOTED?'':', ENT_QUOTES';return"htmlSpecialChars($s$quote)";case
Compiler::CONTEXT_COMMENT:return"HttpPHPUnit\\Nette\\Templating\\Helpers::escapeHtmlComment($s)";case
Compiler::CONTENT_JS:case
Compiler::CONTENT_CSS:return'HttpPHPUnit\Nette\Templating\Helpers::escape'.ucfirst($context[0])."($s)";default:return"HttpPHPUnit\\Nette\\Templating\\Helpers::escapeHtml($s, ENT_NOQUOTES)";}case
Compiler::CONTENT_XML:case
Compiler::CONTENT_JS:case
Compiler::CONTENT_CSS:case
Compiler::CONTENT_ICAL:return'HttpPHPUnit\Nette\Templating\Helpers::escape'.ucfirst($this->compiler->getContentType())."($s)";case
Compiler::CONTENT_TEXT:return$s;default:return"\$template->escape($s)";}}}class
Token
extends
Nette\Object{const
TEXT='text',MACRO_TAG='macroTag',HTML_TAG_BEGIN='htmlTagBegin',HTML_TAG_END='htmlTagEnd',HTML_ATTRIBUTE='htmlAttribute',COMMENT='comment';public$type;public$text;public$line;public$name;public$value;public$modifiers;public$closing;}}namespace HttpPHPUnit\Nette\Loaders{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Caching\Cache;class
RobotLoader
extends
...
tryLoad($type){$type=ltrim(strtolower($type),'\\');$info=&$this->classes[$type];if(isset($this->missing[$type])||(is_int($info)&&$info>=self::RETRY_LIMIT)){return;}if($this->autoRebuild){if(!is_array($info)||!is_file($info['file'])){$info=is_int($info)?$info+1:0;if($this->rebuilt){$this->getCache()->save($this->getKey(),$this->classes,array(Cache::CONSTS=>'HttpPHPUnit\Nette\Framework::REVISION'));}else{$this->rebuild();}}elseif(!$this->rebuilt&&filemtime($info['file'])!==$info['time']){$this->updateFile($info['file']);if(!isset($this->classes[$type])){$this->classes[$type]=0;}$this->getCache()->save($this->getKey(),$this->classes,array(Cache::CONSTS=>'HttpPHPUnit\Nette\Framework::REVISION'));}}if(isset($this->classes[$type]['file'])){Nette\Utils\LimitedScope::load($this->classes[$type]['file'],TRUE);self::$count++;}else{$this->missing[$type]=TRUE;}}function
...
Nette\InvalidStateException("Ambiguous class $class resolution; defined in {$info['file']} and in $file.");{throw$e;}}$info=array('file'=>$file,'time'=>filemtime($file),'orig'=>$class);}}}$dp=array(Cache::CONSTS=>'HttpPHPUnit\Nette\Framework::REVISION');$this->classes+=$missing;return$this->classes;}private
function
...
function
getKey(){return
array($this->ignoreDirs,$this->acceptFiles,$this->scanDirs);}}}namespace HttpPHPUnit\Nette\Mail{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Utils\Strings;class
MimePart
extends
...
static$defaultMailer='HttpPHPUnit\Nette\Mail\SendmailMailer';public
...
SmtpException
extends\Exception{}}namespace HttpPHPUnit\Nette\Reflection{use
HttpPHPUnit\Nette;class
Annotation
extends
Nette\Object
...
HttpPHPUnit\Nette\Utils\Strings;/**
...
__isset($name){return
ObjectMixin::has($this,$name);}function
__unset($name){ObjectMixin::remove($this,$name);}}}namespace HttpPHPUnit\Nette\Security{use
HttpPHPUnit\Nette;class
AuthenticationException
extends\Exception{}}namespace HttpPHPUnit\Nette\Security\Diagnostics{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Diagnostics\Helpers;class
UserPanel
extends
...
<?php
return
ob_get_clean();}}}namespace HttpPHPUnit\Nette\Security{use
HttpPHPUnit\Nette;class
Identity
extends
Nette\FreezableObject
...
function
getAuthenticator(){return$this->authenticator?:$this->context->getByType('HttpPHPUnit\Nette\Security\IAuthenticator');}function
...
function
getAuthorizator(){return$this->authorizator?:$this->context->getByType('HttpPHPUnit\Nette\Security\IAuthorizator');}function
...
setAuthorizationHandler($v){trigger_error(__METHOD__.'() is deprecated; use setAuthorizator() instead.',E_USER_WARNING);return$this->setAuthorizator($v);}}}namespace HttpPHPUnit\Nette\Templating{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Caching;class
Template
extends
Nette\Object
implements
...
getSource(){return$this->source;}function
render(){$cache=new
Caching\Cache($storage=$this->getCacheStorage(),'Nette.Template');$cached=$compiled=$cache->load($this->source);if($compiled===NULL){$compiled=$this->compile();$cache->save($this->source,$compiled,array(Caching\Cache::CONSTS=>'HttpPHPUnit\Nette\Framework::REVISION'));$cached=$cache->load($this->source);}if($cached!==NULL&&$storage
instanceof
...
instanceof
Caching\Storages\PhpFileStorage){$storage->hint=str_replace(dirname(dirname($this->file)),'',$this->file);}$cached=$compiled=$cache->load($this->file);if($compiled===NULL){try{$compiled="<?php\n\n// source file: $this->file\n\n?>".$this->compile();}catch(FilterException$e){$e->setSourceFile($this->file);throw$e;}$cache->save($this->file,$compiled,array(Caching\Cache::FILES=>$this->file,Caching\Cache::CONSTS=>'HttpPHPUnit\Nette\Framework::REVISION'));$cached=$cache->load($this->file);}if($cached!==NULL&&$storage
instanceof
...
HttpPHPUnit\Nette\Utils\Strings;use
HttpPHPUnit\Nette\Forms\Form;use
HttpPHPUnit\Nette\Utils\Html;final
class
Helpers{private
static$helpers=array('normalize'=>'HttpPHPUnit\Nette\Utils\Strings::normalize','toascii'=>'HttpPHPUnit\Nette\Utils\Strings::toAscii','webalize'=>'HttpPHPUnit\Nette\Utils\Strings::webalize','truncate'=>'HttpPHPUnit\Nette\Utils\Strings::truncate','lower'=>'HttpPHPUnit\Nette\Utils\Strings::lower','upper'=>'HttpPHPUnit\Nette\Utils\Strings::upper','firstupper'=>'HttpPHPUnit\Nette\Utils\Strings::firstUpper','capitalize'=>'HttpPHPUnit\Nette\Utils\Strings::capitalize','trim'=>'HttpPHPUnit\Nette\Utils\Strings::trim','padleft'=>'HttpPHPUnit\Nette\Utils\Strings::padLeft','padright'=>'HttpPHPUnit\Nette\Utils\Strings::padRight','reverse'=>'HttpPHPUnit\Nette\Utils\Strings::reverse','replacere'=>'HttpPHPUnit\Nette\Utils\Strings::replace','url'=>'rawurlencode','striptags'=>'strip_tags','nl2br'=>'nl2br','substr'=>'HttpPHPUnit\Nette\Utils\Strings::substring','repeat'=>'str_repeat','implode'=>'implode','number'=>'number_format');public
...
getLength(){return$this->itemCount===NULL?$this->itemsPerPage:min($this->itemsPerPage,$this->itemCount-$this->getPageIndex()*$this->itemsPerPage);}}}namespace HttpPHPUnit\Nette\Utils\PhpGenerator{use
HttpPHPUnit\Nette;class
ClassType
extends
...
__call($name,$args){return
Nette\ObjectMixin::callProperty($this,$name,$args);}}}namespace HttpPHPUnit\Nette\Utils{use
HttpPHPUnit\Nette;use
HttpPHPUnit\Nette\Diagnostics\Debugger;class
Strings{final
function
...
static$counters=array('string'=>'strlen','unicode'=>array('HttpPHPUnit\Nette\Utils\Strings','length'),'array'=>'count','list'=>'count','alnum'=>'strlen','alpha'=>'strlen','digit'=>'strlen','lower'=>'strlen','space'=>'strlen','upper'=>'strlen','xdigit'=>'strlen');static
function
...
AssertionException
extends\Exception{}}namespace HttpPHPUnit\Nette {use
HttpPHPUnit\Nette;
Nette\Diagnostics\Debugger::_init();Nette\Utils\SafeStream::register();function
callback($callback,$m=NULL){return
new
Nette\Callback($callback,$m);}use
HttpPHPUnit\Nette\Diagnostics\Debugger;function
