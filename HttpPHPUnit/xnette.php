<?php //netteloader=XNette\Framework

namespace XNette {

/**
 * XNette Framework (version 2.0-dev released on 2011-05-18, http://nette.org)
 *
...
@header('X-Powered-By: XNette Framework');

define('XNETTE', TRUE);
define('XNETTE_DIR', __DIR__);
define('XNETTE_VERSION_ID', 20000);
define('XNETTE_PACKAGE', '5.3');

}

namespace XNette\Diagnostics {
...
interface IResponse
{
	...
	function send(XNette\Http\IRequest $httpRequest, XNette\Http\IResponse $httpResponse);
...
interface IRouter
{
	...
	function match(XNette\Http\IRequest $httpRequest);

	function constructUrl(Request $appRequest, XNette\Http\Url $refUrl);
...
namespace XNette\ComponentModel {
...
namespace XNette\Application\UI {
...
namespace XNette\Caching\Storages {
...
interface IUser
{
	...
	function setAuthenticationHandler(XNette\Security\IAuthenticator $handler);
	...
	function setAuthorizationHandler(XNette\Security\IAuthorizator $handler);
...
final class LimitedScope
{
	...
	{
		throw new XNette\StaticClassException;
	}
...
namespace XNette\Loaders {

use XNette;

abstract class AutoLoader extends XNette\Object
{
	...
	final static function load($type)
	{
		foreach (func_get_args() as $type) {
			if (!class_exists($type)) {
				throw new XNette\InvalidStateException("Unable to load class or interface '$type'.");
			}
		}
	}
	...
	function register()
	{
		if (!function_exists('spl_autoload_register')) {
			throw new XNette\NotSupportedException('spl_autoload does not exist in this PHP installation.');
		}
...
class Application extends XNette\Object
{
	...
	function __construct(XNette\DI\IContainer $context)
	{
		$this->context = $context;
	}
	...
	function run()
	{
		...
		if ($this->allowedMethods) {
			...
			if (!in_array($method, $this->allowedMethods, TRUE)) {
				$httpResponse->setCode(XNette\Http\IResponse::S501_NOT_IMPLEMENTED);
			...
			try {
				...
				if (!$repeatedError && $this->errorPresenter) {
					...
					if ($e instanceof BadRequestException) {
						...
						$code = 500;
						XNette\Diagnostics\Debugger::log($e, XNette\Diagnostics\Debugger::ERROR);
					}
...
<meta name=robots content=noindex><meta name=generator content="XNette Framework">
...
<?php
	...
	{
		$session = $this->context->session->getNamespace('XNette.Application/requests');
		do {
			$key = XNette\Utils\Strings::random(5);
...
namespace XNette\Application\Diagnostics {

use XNette;use XNette\Application\Routers;use XNette\Application\UI\Presenter;
use XNette\Diagnostics\Debugger;

class RoutingPanel extends XNette\Object implements XNette\Diagnostics\IBarPanel
{
	...
	static function initialize(XNette\Application\Application $application, XNette\Http\IRequest $httpRequest)
	{
		Debugger::$bar->addPanel(new self($application->getRouter(), $httpRequest));
		Debugger::$blueScreen->addPanel(function($e) use($application) {
			if ($e === NULL) {
				return array(
					'tab' => 'XNette Application',
					'panel' => '<h3>Requests</h3>' . XNette\Diagnostics\Helpers::clickableDump($application->getRequests())
						. '<h3>Presenter</h3>' . XNette\Diagnostics\Helpers::clickableDump($application->getPresenter())
				);
			}
		});
	}

	function __construct(XNette\Application\IRouter $router, XNette\Http\IRequest $httpRequest)
	{
		$this->router = $router;
...
class PresenterFactory implements IPresenterFactory
{
	...
	function __construct($baseDir, XNette\DI\IContainer $context)
	{
	...
	function getPresenterClass(& $name)
	{
		...
		if (!is_string($name) || !XNette\Utils\Strings::match($name, "#^[a-zA-Z\x7f-\xff][a-zA-Z0-9\x7f-\xff:]*$#")) {
		...
		if (!class_exists($class)) {
			...
			if (is_file($file) && is_readable($file)) {
				XNette\Utils\LimitedScope::load($file);
			}
		...
		$reflection = new XNette\Reflection\ClassType($class);
		$class = $reflection->getName();

		if (!$reflection->implementsInterface('XNette\Application\IPresenter')) {
			throw new InvalidPresenterException("Cannot load presenter '$name', class '$class' is not XNette\\Application\\IPresenter implementor.");
		}
...
final class Request extends XNette\FreezableObject
{
...
namespace XNette\Application\Responses {

use XNette;

class FileResponse extends XNette\Object implements XNette\Application\IResponse
{
	...
	{
		if (!is_file($file)) {
			throw new XNette\Application\BadRequestException("File '$file' doesn't exist.");
		}
...
class ForwardResponse extends XNette\Object implements XNette\Application\IResponse
{

	private $request;

	function __construct(XNette\Application\Request $request)
	{
		$this->request = $request;
	}
...
class JsonResponse extends XNette\Object implements XNette\Application\IResponse
{
	...
	function __construct($payload, $contentType = NULL)
	{
		if (!is_array($payload) && !is_object($payload)) {
			throw new XNette\InvalidArgumentException("Payload must be array or object class, " . gettype($payload) . " given.");
		}
	...
	function send(XNette\Http\IRequest $httpRequest, XNette\Http\IResponse $httpResponse)
	{
		...
		echo XNette\Utils\Json::encode($this->payload);
	}
...
class RedirectResponse extends XNette\Object implements XNette\Application\IResponse
{
...
class TextResponse extends XNette\Object implements XNette\Application\IResponse
{
	...
	function send(XNette\Http\IRequest $httpRequest, XNette\Http\IResponse $httpResponse)
	{
		if ($this->source instanceof XNette\Templating\ITemplate) {
			$this->source->render();
...
namespace XNette\Application\Routers {

use XNette;use XNette\Application;

class CliRouter extends XNette\Object implements Application\IRouter
{
	...
	function match(XNette\Http\IRequest $httpRequest)
	{
		...
		if (!isset($params[self::PRESENTER_KEY])) {
			throw new XNette\InvalidStateException('Missing presenter & action in route definition.');
		}
	...
	function constructUrl(Application\Request $appRequest, XNette\Http\Url $refUrl)
	{
		return NULL;
	}
...
use XNette\Utils\Strings;

class Route extends XNette\Object implements Application\IRouter
{
	...
	function __construct($mask, $metadata = array(), $flags = 0)
	{
		if (is_string($metadata)) {
			$a = strrpos($metadata, ':');
			if (!$a) {
				throw new XNette\InvalidArgumentException("Second argument must be array or string in format Presenter:action, '$metadata' given.");
			}
			$metadata = array(
	...
	function match(XNette\Http\IRequest $httpRequest)
	{
		...
		if (!isset($params[self::PRESENTER_KEY])) {
			throw new XNette\InvalidStateException('Missing presenter in route definition.');
		}
		if (isset($this->metadata[self::MODULE_KEY])) {
			if (!isset($params[self::MODULE_KEY])) {
				throw new XNette\InvalidStateException('Missing module in route definition.');
			}
	...
	private function setMask($mask, array $metadata)
	{
			...
			foreach ($matches as $match) {
				...
				if ($class !== '') {
					if (!isset(self::$styles[$class])) {
						throw new XNette\InvalidStateException("Parameter '$name' has '$class' flag, but Route::\$styles['$class'] is not set.");
					}
		...
		do {
			...
			if ($part === '[' || $part === ']' || $part === '[!') {
				$brackets += $part[0] === '[' ? -1 : 1;
				if ($brackets < 0) {
					throw new XNette\InvalidArgumentException("Unexpected '$part' in mask '$mask'.");
				}
			...
			if (preg_match('#[^a-z0-9_-]#i', $name)) {
				throw new XNette\InvalidArgumentException("Parameter name must be alphanumeric string due to limitations of PCRE, '$name' given.");
			}
		...
		if ($brackets) {
			throw new XNette\InvalidArgumentException("Missing closing ']' in mask '$mask'.");
		}
	...
	static function addStyle($style, $parent = '#')
	{
		if (isset(self::$styles[$style])) {
			throw new XNette\InvalidArgumentException("Style '$style' already exists.");
		}

		if ($parent !== NULL) {
			if (!isset(self::$styles[$parent])) {
				throw new XNette\InvalidArgumentException("Parent style '$parent' doesn't exist.");
			}
	...
	{
		if (!isset(self::$styles[$style])) {
			throw new XNette\InvalidArgumentException("Style '$style' doesn't exist.");
		}
...
class RouteList extends XNette\ArrayList implements XNette\Application\IRouter
{
	...
	function constructUrl(XNette\Application\Request $appRequest, XNette\Http\Url $refUrl)
	{
	...
	{
		if (!$route instanceof XNette\Application\IRouter) {
			throw new XNette\InvalidArgumentException("Argument must be IRouter descendant.");
		}
...
use XNette\Application;

class SimpleRouter extends XNette\Object implements Application\IRouter
{
	const PRESENTER_KEY = 'presenter';
	const MODULE_KEY = 'module';
	...
	function __construct($defaults = array(), $flags = 0)
	{
		if (is_string($defaults)) {
			$a = strrpos($defaults, ':');
			if (!$a) {
				throw new XNette\InvalidArgumentException("Argument must be array or string in format Presenter:action, '$defaults' given.");
			}
			$defaults = array(
	...
	function match(XNette\Http\IRequest $httpRequest)
	{
		...
		if (!isset($params[self::PRESENTER_KEY])) {
			throw new XNette\InvalidStateException('Missing presenter.');
		}
...
class BadSignalException extends XNette\Application\BadRequestException
{
...
abstract class Component extends XNette\Object implements IComponent
{
	...
	function lookup($type, $need = TRUE)
	{
		...
		if ($need && $this->monitors[$type][0] === NULL) {
			throw new XNette\InvalidStateException("Component '$this->name' is not attached to '$type'.");
		}
	...
	function setParent(IContainer $parent = NULL, $name = NULL)
	{
		...
		if ($this->parent !== NULL && $parent !== NULL) {
			throw new XNette\InvalidStateException("Component '$this->name' already has a parent.");
		}
	...
	{
		throw new XNette\NotImplementedException;
	}
...
class Container extends Component implements IContainer
{
	...
	function addComponent(IComponent $component, $name, $insertBefore = NULL)
	{
		...
		if (is_int($name)) {
			...
			throw new XNette\InvalidArgumentException("Component name must be integer or string, " . gettype($name) . " given.");

		} elseif (!preg_match('#^[a-zA-Z0-9_]+$#', $name)) {
			throw new XNette\InvalidArgumentException("Component name must be non-empty alphanumeric string, '$name' given.");
		}

		if (isset($this->components[$name])) {
			throw new XNette\InvalidStateException("Component with name '$name' already exists.");
		}

		$obj = $this;
		do {
			if ($obj === $component) {
				throw new XNette\InvalidStateException("Circular reference detected while adding component '$name'.");
			}
			$obj = $obj->getParent();
		} while ($obj !== NULL);
	...
	function removeComponent(IComponent $component)
	{
		...
		if (!isset($this->components[$name]) || $this->components[$name] !== $component) {
			throw new XNette\InvalidArgumentException("Component named '$name' is not located in this container.");
		}
	...
	final function getComponent($name, $need = TRUE)
	{
		if (is_int($name)) {
			...
			if ($name === '') {
				throw new XNette\InvalidArgumentException("Component or subcomponent name must not be empty string.");
			}
		}
		...
		if (isset($this->components[$name])) {
			if (!isset($ext)) {
				...
				throw new XNette\InvalidArgumentException("Component with name '$name' is not container and cannot have '$ext' component.");
			}

		} elseif ($need) {
			throw new XNette\InvalidArgumentException("Component with name '$name' does not exist.");
		}
	}
	...
	protected function createComponent($name)
	{
		...
		if ($ucname !== $name && method_exists($this, $method) && $this->getReflection()->getMethod($method)->getName() === $method) {
			...
			if (!$component instanceof IComponent && !isset($this->components[$name])) {
				$class = get_class($this);
				throw new XNette\UnexpectedValueException("Method $class::$method() did not return or create the desired component.");
			}
			return $component;
		}
	}
	...
	final function getComponents($deep = FALSE, $filterType = NULL)
	{
		...
		if ($filterType) {
			$iterator = new XNette\Iterators\InstanceFilter($iterator, $filterType);
		}
		return $iterator;
	}
...
abstract class PresenterComponent extends XNette\ComponentModel\Container implements ISignalReceiver, IStatePersistent, \ArrayAccess
{

	protected $params = array();

	function __construct(XNette\ComponentModel\IContainer $parent = NULL, $name = NULL)
	{
		$this->monitor('XNette\Application\UI\Presenter');
		parent::__construct($parent, $name);
	}

	function getPresenter($need = TRUE)
	{
		return $this->lookup('XNette\Application\UI\Presenter', $need);
	}

	function getUniqueId()
	{
		return $this->lookupPath('XNette\Application\UI\Presenter', TRUE);
	}
	...
	function saveState(array & $params, $reflection = NULL)
	{
		...
		foreach ($reflection->getPersistentParams() as $nm => $meta) {
			...
			if (is_object($val)) {
				$class = get_class($this);
				throw new XNette\InvalidStateException("Persistent parameter must be scalar or array, $class::\$$nm is " . gettype($val));
	...
	{
		$rc = new XNette\Reflection\ClassType(get_called_class());
		$params = array();
...
abstract class Control extends PresenterComponent implements IPartiallyRenderable
{
	...
	final function getTemplate()
	{
		if ($this->template === NULL) {
			...
			if (!$value instanceof XNette\Templating\ITemplate && $value !== NULL) {
				$class2 = get_class($value); $class = get_class($this);
				throw new XNette\UnexpectedValueException("Object returned by $class::createTemplate() must be instance of XNette\\Templating\\ITemplate, '$class2' given.");
			}
			$this->template = $value;
		}
	...
	{
		$template = new XNette\Templating\FileTemplate;
		$presenter = $this->getPresenter(FALSE);
		$template->onPrepareFilters[] = \XNette\callback($this, 'templatePrepareFilters');
		...
		$template->registerHelper('escape', 'XNette\Templating\DefaultHelpers::escapeHtml');
		...
		$template->registerHelper('replaceRE', 'XNette\Utils\Strings::replace');
		$template->registerHelper('implode', 'implode');
		$template->registerHelper('number', 'number_format');
		$template->registerHelperLoader('XNette\Templating\DefaultHelpers::loader');
	...
	function templatePrepareFilters($template)
	{
		...
		$template->registerFilter(new XNette\Latte\Engine);
	}
...
class Container extends XNette\ComponentModel\Container implements \ArrayAccess
{
	...
	function setValues($values, $erase = FALSE)
	{
		if ($values instanceof \Traversable) {
			...
			throw new XNette\InvalidArgumentException("First parameter must be an array, " . gettype($values) ." given.");
		}
	...
	function addComponent(XNette\ComponentModel\IComponent $component, $name, $insertBefore = NULL)
	{
	...
	{
		return $this->getComponents(TRUE, 'XNette\Forms\IControl');
	}

	function getForm($need = TRUE)
	{
		return $this->lookup('XNette\Forms\Form', $need);
	}
	...
	{
		throw new XNette\NotImplementedException('Form cloning is not supported yet.');
	}
...
class Form extends Container
{
	...
	const PROTECTION = 'XNette\Forms\Controls\HiddenField::validateEqual';
	...
	{
		$this->element = XNette\Utils\Html::el('form');
		$this->element->action = '';
	...
	protected function attached($obj)
	{
		if ($obj instanceof self) {
			throw new XNette\InvalidStateException('Nested forms are forbidden.');
		}
	}
	...
	function setMethod($method)
	{
		if ($this->httpData !== NULL) {
			throw new XNette\InvalidStateException(__METHOD__ . '() must be called until the form is empty.');
		}
	...
	{
		$session = $this->getSession()->getNamespace('XNette.Forms.Form/CSRF');
		$key = "key$timeout";
		if (isset($session->$key)) {
			$token = $session->$key;
		} else {
			$session->$key = $token = XNette\Utils\Strings::random();
		}
	...
	function removeGroup($name)
	{
		if (is_string($name) && isset($this->groups[$name])) {
			...
			throw new XNette\InvalidArgumentException("Group not found in form '$this->name'");
		}
	...
	function setTranslator(XNette\Localization\ITranslator $translator = NULL)
	{
	...
	final function getHttpData()
	{
		if ($this->httpData === NULL) {
			if (!$this->isAnchored()) {
				throw new XNette\InvalidStateException('Form is not anchored and therefore can not determine whether it was submitted.');
			}
	...
	protected function receiveHttpData()
	{
		...
		if ($httpRequest->isMethod('post')) {
			$data = XNette\Utils\Arrays::mergeTree($httpRequest->getPost(), $httpRequest->getFiles());
		} else {
	...
	function __toString()
	{
		try {
			...
			if (func_get_args() && func_get_arg(0)) {
				...
				XNette\Diagnostics\Debugger::toStringException($e);
			}
		}
	}
	...
	{
		return XNette\Environment::getHttpRequest();
	}

	protected function getSession()
	{
		return XNette\Environment::getSession();
	}
...
class Form extends XNette\Forms\Form implements ISignalReceiver
{
	...
	{
		if ($presenter instanceof Presenter) {
			$name = $this->lookupPath('XNette\Application\UI\Presenter');
	...
	protected function receiveHttpData()
	{
		...
		if ($isPost) {
			return XNette\Utils\Arrays::mergeTree($request->getPost(), $request->getFiles());
		} else {
...
class Link extends XNette\Object
{
...
use XNette\Application;use XNette\Application\Responses;use XNette\Http;use XNette\Reflection;
...
abstract class Presenter extends Control implements Application\IPresenter
{
	...
	function run(Application\Request $request)
	{
		try {
			...
			if (!$this->startupCheck) {
				...
				throw new XNette\InvalidStateException("Method $class::startup() or its descendant doesn't call parent::startup().");
			}
	...
	{
		if ($component instanceof XNette\ComponentModel\Component) {
	...
	{
		if (XNette\Utils\Strings::match($action, "#^[a-zA-Z0-9][a-zA-Z0-9_\x7f-\xff]*$#")) {
			$this->action = $action;
			$this->view = $action;
	...
	function sendTemplate()
	{
		...
		if ($template instanceof XNette\Templating\IFileTemplate && !$template->getFile()) {
		...
		if ($this->layout !== FALSE) {
			...
			if (empty($template->layout) && $this->layout !== NULL) {
				$file = preg_replace('#^.*([/\\\\].{1,70})$#U', "\xE2\x80\xA6\$1", reset($files));
				$file = strtr($file, '/', DIRECTORY_SEPARATOR);
				throw new XNette\FileNotFoundException("Layout not found. Missing template '$file'.");
			}
		}
	...
	private function getGlobalState($forClass = NULL)
	{
		...
		if ($this->globalState === NULL) {
			...
			$iterator = $this->getComponents(TRUE, 'XNette\Application\UI\IStatePersistent');
	...
	function hasFlashSession()
	{
		return !empty($this->params[self::FLASH_KEY])
			&& $this->getSession()->hasNamespace('XNette.Application.Flash/' . $this->params[self::FLASH_KEY]);
	}

	function getFlashSession()
	{
		if (empty($this->params[self::FLASH_KEY])) {
			$this->params[self::FLASH_KEY] = XNette\Utils\Strings::random(4);
		}
		return $this->getSession('XNette.Application.Flash/' . $this->params[self::FLASH_KEY]);
	}

	function setContext(XNette\DI\IContainer $context)
	{
		$this->context = $context;
		return $this;
	}
...
use XNette;use XNette\ObjectMixin;
...
class ClassType extends \ReflectionClass
{
	...
	function setExtensionMethod($name, $callback)
	{
		...
		$l[strtolower($this->getName())] = \XNette\callback($callback);
		$l[''] = NULL;
		return $this;
	}
...
class PresenterComponentReflection extends XNette\Reflection\ClassType
{
	...
	function getPersistentParams($class = NULL)
	{
		...
		$params = array();
		if (is_subclass_of($class, 'XNette\Application\UI\PresenterComponent')) {
	...
	function getPersistentComponents()
	{
		...
		$components = array();
		if (is_subclass_of($class, 'XNette\Application\UI\Presenter')) {
	...
	function hasCallableMethod($method)
	{
		...
		if ($cache === NULL) try {
			$cache = FALSE;
			$rm = XNette\Reflection\Method::from($class, $method);
...
class Cache extends XNette\Object implements \ArrayAccess
{
	...
	function save($key, $data, array $dp = NULL)
	{
		...
		if (isset($dp[Cache::EXPIRATION])) {
			$dp[Cache::EXPIRATION] = XNette\DateTime::from($dp[Cache::EXPIRATION])->format('U') - time();
		}
		...
		if ($data instanceof XNette\Callback || $data instanceof \Closure) {
			XNette\Utils\CriticalSection::enter();
			$data = $data->__invoke();
			XNette\Utils\CriticalSection::leave();
		}

		if (is_object($data)) {
			$dp[self::CALLBACKS][] = array(array(__CLASS__, 'checkSerializationVersion'), get_class($data),
				XNette\Reflection\ClassType::from($data)->getAnnotation('serializationVersion'));
		}
	...
	{
		return XNette\Reflection\ClassType::from($class)->getAnnotation('serializationVersion') === $value;
	}

}

class OutputHelper extends XNette\Object
{
	...
	{
		if ($this->cache === NULL) {
			throw new XNette\InvalidStateException('Output cache has already been saved.');
		}
...
class DevNullStorage extends XNette\Object implements XNette\Caching\IStorage
{
...
use XNette\Caching\Cache;

class FileJournal extends XNette\Object implements IJournal
{
	...
	function __construct($dir)
	{
		...
		if (!file_exists($this->file)) {
			...
			if (!$init) {
				clearstatcache();
				if (!file_exists($this->file)) {
					throw new XNette\InvalidStateException("Cannot create journal file $this->file.");
				}
			} else {
				$writen = fwrite($init, pack('N2', self::FILE_MAGIC, $this->lastNode));
				fclose($init);
				if ($writen !== self::INT32_SIZE * 2) {
					throw new XNette\InvalidStateException("Cannot write journal header.");
				}
			}
		}
		...
		if (!$this->handle) {
			throw new XNette\InvalidStateException("Cannot open journal file '$this->file'.");
		}

		if (!flock($this->handle, LOCK_SH)) {
			throw new XNette\InvalidStateException('Cannot acquire shared lock on journal.');
		}
		...
		if ($fileMagic !== self::FILE_MAGIC) {
			fclose($this->handle);
			$this->handle = false;
			throw new XNette\InvalidStateException("Malformed journal file '$this->file'.");
		}
	}
	...
	private function cleanPriority($priority, array &$toDelete)
	{
		...
		while ($nodeInfo[self::PREV_NODE] !== -1) {
			...
			if ($node === FALSE) {
				if (self::$debug) {
					throw new XNette\InvalidStateException("Cannot load node number $nodeId.");
				}
				break;
			}
	...
	private function cleanLinks(array $data, array &$toDelete)
	{
		...
		while ($i < $max) {
			...
			do {
				...
				if (!isset($node[$link])) {
					if (self::$debug) {
						throw new XNette\InvalidStateException("Link with ID $searchLink is not in node ". ($nodeId) . '.');
					}
					continue;
		...
		foreach ($toDeleteFromIndex as $type => $toDelete) {
			...
			while (!empty($toDelete)) {
				...
				if (!isset($masterNode[$searchKey])) {
					if (self::$debug) {
						throw new XNette\InvalidStateException('Bad index.');
					}
	...
	private function mergeIndexData(array $data)
	{
		while (isset($data[self::INDEX_DATA])) {
			...
			if ($childNode === FALSE) {
				if (self::$debug) {
					throw new XNette\InvalidStateException("Cannot load node number $id.");
				}
				break;
			}
		...
		while ($nextNodeId && !empty($links)) {
			...
			if (empty($node[self::INDEX_DATA]) || (count($node[self::INDEX_DATA]) === 1 && $nextNodeId)) {
				if ($prev === -1) {
					...
					if ($prevNode === FALSE) {
						if (self::$debug) {
							throw new XNette\InvalidStateException("Cannot load node number $prev.");
						}
					} else {
						if ($nextNodeId === FALSE) {
	...
	private function getNode($id)
	{
		...
		if ($magic !== self::INDEX_MAGIC && $magic !== self::DATA_MAGIC) {
			if (!empty($magic)) {
				if (self::$debug) {
					throw new XNette\InvalidStateException("Node $id has malformed header.");
				}
				$this->deleteNode($id);
			}
			return FALSE;
		...
		if ($node === FALSE) {
			$this->deleteNode($id);
			if (self::$debug) {
				throw new XNette\InvalidStateException("Cannot deserialize node number $id.");
			}
			return FALSE;
		}
		...
		if (count($node) === 1) {
			...
			if ($nodeInfo[self::TYPE] !== self::DATA) {
				...
				if ($parentId !== -1 && $parentId !== $id) {
					...
					if ($parentNode === FALSE) {
						if (self::$debug) {
							throw new XNette\InvalidStateException("Cannot load node number $parentId.");
						}
					} else {
				...
				if ($nodeInfo[self::TYPE] === self::PRIORITY) {
					if ($nodeInfo[self::MAX] === -1) {
						if ($nodeInfo[self::PREV_NODE] !== -1) {
							...
							if ($prevNode === FALSE) {
								if (self::$debug) {
									throw new XNette\InvalidStateException('Cannot load node number ' . $nodeInfo[self::PREV_NODE] . '.');
								}
							} else {
	...
	private function prepareNode($id, $node)
	{
		...
		if ($dataSize > self::NODE_SIZE) {
			if ($isData) {
				throw new XNette\InvalidStateException('Saving node is bigger than maximum node size.');
			} else {
	...
	private function commitNode($id, $str)
	{
		...
		if ($writen === FALSE) {
			throw new XNette\InvalidStateException("Cannot write node number $id to journal.");
		}
	}
	...
	private function bisectNode($id, array $node)
	{
		...
		if ($id <= 2) {
			...
			if ($parentNode === FALSE) {
				if (self::$debug) {
					throw new XNette\InvalidStateException("Cannot load node number $parent.");
				}
			} else {
	...
	private function deleteNode($id)
	{
		...
		if ($end <= (self::HEADER_SIZE + self::NODE_SIZE * ($id + 1))) {
			...
			if (!ftruncate($this->handle, self::HEADER_SIZE + self::NODE_SIZE * ($id + 1))) {
				throw new XNette\InvalidStateException('Cannot truncate journal file.');
			}
		} else {
			fseek($this->handle, self::HEADER_SIZE + self::NODE_SIZE * $id);
			$writen = fwrite($this->handle, pack('N', 0));
			if ($writen !== self::INT32_SIZE) {
				throw new XNette\InvalidStateException("Cannot delete node number $id from journal.");
			}
		}
	}
	...
	{
		if (!$this->handle) {
			throw new XNette\InvalidStateException('File journal file is not opened');
		}

		if (!flock($this->handle, LOCK_EX)) {
			throw new XNette\InvalidStateException('Cannot acquire exclusive lock on journal.');
		}
...
class FileStorage extends XNette\Object implements XNette\Caching\IStorage
{
	...
	function __construct($dir, IJournal $journal = NULL)
	{
		...
		if ($this->dir === FALSE) {
			throw new XNette\DirectoryNotFoundException("Directory '$dir' not found.");
		}
	...
	function write($key, $data, array $dp)
	{
		...
		if (isset($dp[Cache::TAGS]) || isset($dp[Cache::PRIORITY])) {
			if (!$this->journal) {
				throw new XNette\InvalidStateException('CacheJournal has not been provided.');
			}
	...
	function clean(array $conds)
	{
		...
		if ($all || $collector) {
			$now = time();
			foreach (XNette\Utils\Finder::find('*')->from($this->dir)->childFirst() as $entry) {
				$path = (string) $entry;
				if ($entry->isDir()) {
					@rmdir($path);
					continue;
...
class MemcachedStorage extends XNette\Object implements XNette\Caching\IStorage
{
	...
	{
		if (!self::isAvailable()) {
			throw new XNette\NotSupportedException("PHP extension 'memcache' is not loaded.");
		}

		$this->prefix = $prefix;
		$this->journal = $journal;
		$this->memcache = new \Memcache;
		XNette\Diagnostics\Debugger::tryError();
		$this->memcache->connect($host, $port);
		if (XNette\Diagnostics\Debugger::catchError($e)) {
			throw new XNette\InvalidStateException('Memcache::connect(): ' . $e->getMessage(), 0, $e);
		}
	}
	...
	{
		if (isset($dp[Cache::ITEMS])) {
			throw new XNette\NotSupportedException('Dependent items are not supported by MemcachedStorage.');
		}
...
class MemoryStorage extends XNette\Object implements XNette\Caching\IStorage
{
	...
	function clean(array $conds)
	{
		if (!empty($conds[XNette\Caching\Cache::ALL])) {
			$this->data = array();
		}
	}
...
class PhpFileStorage extends FileStorage
{
	...
	protected function getCacheFile($key)
	{
		return parent::getCacheFile(substr_replace(
			...
			strpos($key, XNette\Caching\Cache::NAMESPACE_SEPARATOR) + 1,
			0
		)) . '.php';
	}
...
final class Callback extends Object
{
	...
	{
		if (is_array($this->cb)) {
			return new XNette\Reflection\Method($this->cb[0], $this->cb[1]);
		} else {
			return new XNette\Reflection\GlobalFunction($this->cb);
		}
	}
...
use XNette\Caching\Cache;use XNette\DI;
...
class Configurator extends Object
{
	...
	function __construct($containerClass = 'XNette\DI\Container')
	{
		self::$instance = $this;
	...
	function loadConfig($file, $section = NULL)
	{
		...
		$config = XNette\Config\Config::fromFile($file, $section);
		$code = "<?php\n// source file $file\n\n";
		...
		while (!empty($config['variables'])) {
			...
			foreach ($config['variables'] as $key => $value) {
				...
				} catch (XNette\InvalidArgumentException $e) {}
			}
		...
		if (isset($config['services'])) {
			foreach ($config['services'] as $key => & $def) {
				if (preg_match('#^XNette\\\\.*\\\\I?([a-zA-Z]+)$#', strtr($key, '-', '\\'), $m)) {
		...
		foreach ($config as $key => $value) {
			$code .= $this->generateCode('$container->params[?] = ' . (is_array($value) ? 'XNette\ArrayHash::from(?)' : '?'), $key, $value);
		}
	...
	{
		if (!is_scalar($value)) {
			throw new XNette\InvalidStateException("Configuration value for directive '$name' is not scalar.");
		}
		...
		if (function_exists('ini_set')) {
			...
			throw new XNette\NotSupportedException('Required function ini_set() is disabled.');
		}
	}
	...
	static function createServiceApplication(DI\Container $container, array $options = NULL)
	{
		...
		XNette\Application\UI\Presenter::$invalidLinkMode = $container->params['productionMode']
			? XNette\Application\UI\Presenter::INVALID_LINK_SILENT
			: XNette\Application\UI\Presenter::INVALID_LINK_WARNING;

		$class = isset($options['class']) ? $options['class'] : 'XNette\Application\Application';
	...
	{
		return new XNette\Application\PresenterFactory($container->params['appDir'], $container);
	}

	static function createServiceRouter(DI\Container $container)
	{
		return new XNette\Application\Routers\RouteList;
	}

	static function createServiceHttpRequest()
	{
		$factory = new XNette\Http\RequestFactory;
	...
	{
		return new XNette\Http\Response;
	}

	static function createServiceHttpContext(DI\Container $container)
	{
		return new XNette\Http\Context($container->httpRequest, $container->httpResponse);
	}

	static function createServiceSession(DI\Container $container)
	{
		return new XNette\Http\Session($container->httpRequest, $container->httpResponse);
	}
	...
	static function createServiceUser(DI\Container $container)
	{
		...
		$context->addService('session', $container->session);
		return new XNette\Http\User($context);
	}
	...
	static function createServiceCacheStorage(DI\Container $container)
	{
		...
		umask(0000);
		@mkdir($dir, 0777);
		return new XNette\Caching\Storages\FileStorage($dir, $container->cacheJournal);
	}
	...
	static function preloadEnvironment(DI\Container $container)
	{
		...
		if (!@mkdir("$dir/$uniq", 0777)) {
			throw new XNette\InvalidStateException("Unable to write to directory '$dir'. Make this directory writable.");
		}
		...
		$code .= self::generateCode('XNette\Caching\Storages\FileStorage::$useDirectories = ?', $useDirs);
		return $code;
	}
...
final class Environment
{
	...
	static function __callStatic($name, $args)
	{
		if (!$args && strncasecmp($name, 'get', 3) === 0) {
			...
			throw new MemberAccessException("Call to undefined static method XNette\\Environment::$name().");
		}
	}
...
class Config
{
	...
	private static $extensions = array(
		'ini' => 'XNette\Config\IniAdapter',
		'neon' => 'XNette\Config\NeonAdapter',
	);
	...
	{
		if (!class_exists($class)) {
			throw new XNette\InvalidArgumentException("Class '$class' was not found.");
		}

		if (!XNette\Reflection\ClassType::from($class)->implementsInterface('XNette\Config\IAdapter')) {
			throw new XNette\InvalidArgumentException("Configuration adapter '$class' is not XNette\\Config\\IAdapter implementor.");
		}
	...
	static function fromFile($file, $section = NULL)
	{
		...
		if (!isset(self::$extensions[$extension])) {
			throw new XNette\InvalidArgumentException("Unknown file extension '$file'.");
		}

		$data = call_user_func(array(self::$extensions[$extension], 'load'), $file, $section);
		if ($section) {
			if (!isset($data[$section]) || !is_array($data[$section])) {
				throw new XNette\InvalidStateException("There is not section [$section] in file '$file'.");
			}
			$data = $data[$section];
		}
		return $data;
...
final class IniAdapter implements IAdapter
{
	...
	{
		if (!is_file($file) || !is_readable($file)) {
			throw new XNette\FileNotFoundException("File '$file' is missing or is not readable.");
		}

		XNette\Diagnostics\Debugger::tryError();
		$ini = parse_ini_file($file, TRUE);
		if (XNette\Diagnostics\Debugger::catchError($e)) {
			throw new XNette\InvalidStateException('parse_ini_file(): ' . $e->getMessage(), 0, $e);
		}
		...
		foreach ($ini as $secName => $secData) {
			...
			if (is_array($secData)) {
					...
					foreach ($secData as $key => $val) {
						...
						foreach (explode(self::$keySeparator, $key) as $part) {
							if (!isset($cursor[$part]) || is_array($cursor[$part])) {
								...
								throw new XNette\InvalidStateException("Invalid key '$key' in section [$secName] in file '$file'.");
							}
						}
						$cursor = $val;
					}
				...
				if (count($parts) > 1) {
					...
					if (!isset($data[$parent]) || !is_array($data[$parent])) {
						throw new XNette\InvalidStateException("Missing parent section [$parent] in file '$file'.");
					}
					$secData = XNette\Utils\Arrays::mergeTree($secData, $data[$parent]);
					$secName = trim($parts[0]);
					if ($secName === '') {
						throw new XNette\InvalidStateException("Invalid empty section name in file '$file'.");
					}
				}
			}
			...
			if (self::$keySeparator) {
				...
				foreach (explode(self::$keySeparator, $secName) as $part) {
					if (!isset($cursor[$part]) || is_array($cursor[$part])) {
						...
						throw new XNette\InvalidStateException("Invalid section [$secName] in file '$file'.");
					}
				}
			} else {
				$cursor = & $data[$secName];
			...
			if (is_array($secData) && is_array($cursor)) {
				$secData = XNette\Utils\Arrays::mergeTree($secData, $cursor);
			}
	...
	{
		$output = array();
		$output[] = '; generated by XNette';
		$output[] = '';

		foreach ($config as $secName => $secData) {
			if (!(is_array($secData) || $secData instanceof \Traversable)) {
				throw new XNette\InvalidStateException("Invalid section '$secName'.");
			}
		...
		if (!file_put_contents($file, implode(PHP_EOL, $output))) {
			throw new XNette\IOException("Cannot write file '$file'.");
		}
	}
	...
	private static function build($input, & $output, $prefix)
	{
		foreach ($input as $key => $val) {
			if (is_array($val) || $val instanceof \Traversable) {
				...
				throw new XNette\InvalidArgumentException("The '$prefix$key' item must be scalar or array, " . gettype($val) ." given.");
			}
		}
	}
...
final class NeonAdapter implements IAdapter
{
	...
	static function save($config, $file)
	{
		if (!file_put_contents($file, "# generated by XNette\n\n" . Neon::encode($config, Neon::BLOCK))) {
			throw new XNette\IOException("Cannot write file '$file'.");
		}
	}
...
use XNette;use XNette\ObjectMixin;use PDO;
...
if (class_exists('PDO')){ class Connection extends PDO
{
	...
	function __construct($dsn, $username = NULL, $password  = NULL, array $options = NULL)
	{
		...
		$this->setAttribute(PDO::ATTR_STATEMENT_CLASS, array('XNette\Database\Statement', array($this)));

		$class = 'XNette\Database\Drivers\\' . $this->getAttribute(PDO::ATTR_DRIVER_NAME) . 'Driver';
		if (class_exists($class)) {
	...
	function loadFile($file)
	{
		...
		$handle = @fopen($file, 'r');
		if (!$handle) {
			throw new XNette\FileNotFoundException("Cannot open file '$file'.");
		}
	...
	static function getReflection()
	{
		return new XNette\Reflection\ClassType(get_called_class());
	}
...
namespace XNette\Database\Diagnostics {

use XNette;use XNette\Database\Connection;use XNette\Diagnostics\Debugger;

class ConnectionPanel extends XNette\Object implements XNette\Diagnostics\IBarPanel
{
	...
	static function initialize(Connection $connection)
	{
		if (!Debugger::$productionMode) {
			$panel = new static;
			$connection->onQuery[] = \XNette\callback($panel, 'logQuery');
			Debugger::$bar->addPanel($panel);
			Debugger::$blueScreen->addPanel(\XNette\callback($panel, 'renderException'), __CLASS__);
		}
	}

	function logQuery(XNette\Database\Statement $result, array $params = NULL)
	{
		if ($this->disabled) {
			return;
		}
		...
		foreach (debug_backtrace(FALSE) as $row) {
			if (isset($row['file']) && is_file($row['file']) && strpos($row['file'], XNETTE_DIR . DIRECTORY_SEPARATOR) !== 0) {
	...
	function getTab()
	{
		return '<span title="XNette\\Database ' . htmlSpecialChars($this->name) . '">'
	...
	function getPanel()
	{
		...
		foreach ($this->queries as $i => $query) {
			...
			if ($explain) {
				static $counter;
				$counter++;
				$s .= "<br /><a href='#' class='xnette-toggler' rel='#nette-DbConnectionPanel-row-$counter'>explain&nbsp;&#x25ba;</a>";
			}

			$s .= '</td><td class="nette-DbConnectionPanel-sql">' . Connection::highlightSql(XNette\Utils\Strings::truncate($sql, self::$maxLength));
			if ($explain) {
				$s .= "<table id='xnette-DbConnectionPanel-row-$counter' class='xnette-collapsed'><tr>";
			...
			if ($source) {
				$s .= XNette\Diagnostics\Helpers::editorLink($source[0], $source[1])->class('xnette-DbConnectionPanel-source');
			}
...
namespace XNette\Database\Drivers {

use XNette;

class MsSqlDriver extends XNette\Object implements XNette\Database\ISupplementalDriver
{
	...
	function __construct(XNette\Database\Connection $connection, array $options)
	{
	...
	function applyLimit(&$sql, $limit, $offset)
	{
		...
		if ($offset) {
			throw new XNette\NotImplementedException('Offset is not implemented.');
		}
	}
...
class MySqlDriver extends XNette\Object implements XNette\Database\ISupplementalDriver
{
...
class OciDriver extends XNette\Object implements XNette\Database\ISupplementalDriver
{
...
class OdbcDriver extends XNette\Object implements XNette\Database\ISupplementalDriver
{
	...
	function applyLimit(&$sql, $limit, $offset)
	{
		...
		if ($offset) {
			throw new XNette\InvalidArgumentException('Offset is not implemented in driver odbc.');
		}
	}
...
class PgSqlDriver extends XNette\Object implements XNette\Database\ISupplementalDriver
{
...
class SqliteDriver extends XNette\Object implements XNette\Database\ISupplementalDriver
{
...
class Sqlite2Driver extends SqliteDriver
{
	...
	function formatLike($value, $pos)
	{
		throw new XNette\NotSupportedException;
	}
...
namespace XNette\Database\Reflection {

use XNette;

class DatabaseReflection extends XNette\Object
{
...
class Row extends XNette\ArrayHash
{
...
class SqlPreprocessor extends XNette\Object
{
	...
	function process($sql, $params)
	{
		...
		$sql = XNette\Utils\Strings::replace($sql, '~\'.*?\'|".*?"|:[a-zA-Z0-9_]+:|\?~s', array($this, 'callback'));
...
use PDO;use XNette\ObjectMixin;
...
if (class_exists('PDO')){ class Statement extends \PDOStatement
{
	...
	{
		$this->connection = $connection;
		$this->setFetchMode(PDO::FETCH_CLASS, 'XNette\Database\Row', array($this));
	}
...
namespace XNette\Database\Table {

use XNette;

class ActiveRow extends XNette\Object implements \IteratorAggregate, \ArrayAccess
{
...
class Selection extends XNette\Object implements \Iterator, \ArrayAccess, \Countable
{
	...
	function __construct($table, XNette\Database\Connection $connection)
	{
		$this->name = $table;
		$this->connection = $connection;
	...
	function update($data)
	{
		if ($data instanceof \Traversable) {
			...
			throw new XNette\InvalidArgumentException;
		}
...
class Container extends XNette\FreezableObject implements IContainer
{
	...
	function addService($name, $service, $tags = NULL)
	{
		...
		if (!is_string($name) || $name === '') {
			throw new XNette\InvalidArgumentException("Service name must be a non-empty string, " . gettype($name) . " given.");
		}

		if (isset($this->registry[$name]) || method_exists($this, "createService$name")) {
			throw new XNette\InvalidStateException("Service '$name' has already been registered.");
		}
		...
		} elseif (is_object($service) && !$service instanceof \Closure && !$service instanceof XNette\Callback) {
		...
		$this->factories[$name] = array(\XNette\callback($factory));
		$this->tags[$name] = $tags;
	...
	function getService($name)
	{
		if (isset($this->registry[$name])) {
			...
			throw new XNette\InvalidStateException("Circular reference detected for services: "
		...
		if (isset($this->factories[$name])) {
			...
			if (!$factory->isCallable()) {
				throw new XNette\InvalidStateException("Unable to create service '$name', factory '$factory' is not callable.");
			}
		...
		if (isset($e)) {
			...
			throw new XNette\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not object.");

		} elseif (isset($this->tags[$name][self::TAG_TYPEHINT][0]) && !$service instanceof $this->tags[$name][self::TAG_TYPEHINT][0]) {
			throw new XNette\UnexpectedValueException("Unable to create service '$name', value returned by factory '$factory' is not '{$this->tags[$name][self::TAG_TYPEHINT][0]}' type.");
		}
		...
		if (is_string($s) && strpos($s, '%') !== FALSE) {
			...
			return @preg_replace_callback('#%([a-z0-9._-]*)%#i', function ($m) use($that) {
				...
				if ($param === '') {
					...
					throw new XNette\InvalidArgumentException("Missing parameter '$param'.");
				} elseif (!is_scalar($val = $that->params[$param])) {
					throw new XNette\InvalidStateException("Parameter '$param' is not scalar.");
				}
				return $val;
			}, $s);
		}
...
class ContainerBuilder extends XNette\Object
{
			...
			if (isset($definition['class'])) {
				...
				$factory = function($container) use($class, $arguments, $methods, $expander) {
					...
					if ($arguments) {
						...
						$service = XNette\Reflection\ClassType::from($class)->newInstanceArgs($arguments);
					} else {
						$service = new $class;
					}
				...
				throw new XNette\InvalidStateException("Factory method is not specified.");
			}
...
class MissingServiceException extends XNette\InvalidStateException
{
}

class AmbiguousServiceException extends XNette\InvalidStateException
{
}

class ServiceBuilder extends XNette\Object implements IServiceBuilder
{
	...
	function createService(XNette\DI\IContainer $container)
	{
		if (!class_exists($this->class)) {
			throw new XNette\InvalidStateException("Cannot instantiate service, class '$this->class' not found.");
		}
...
class Bar extends XNette\Object
{
...
<script id="nette-debug-script">/*<![CDATA[*/var XNette=XNette||{};
(function(){XNette.Class=function(a){var b=a.constructor||function(){},c,d=Object.prototype.hasOwnProperty;delete a.constructor;if(a.Extends){var f=function(){this.constructor=b};f.prototype=a.Extends.prototype;b.prototype=new f;delete a.Extends}if(a.Static){for(c in a.Static)if(d.call(a.Static,c))b[c]=a.Static[c];delete a.Static}for(c in a)if(d.call(a,c))b.prototype[c]=a[c];return b};XNette.Q=XNette.Class({Static:{factory:function(a){return new XNette.Q(a)},implement:function(a){var b,c=XNette.Q.implement,
d=XNette.Q.prototype,f=Object.prototype.hasOwnProperty;for(b in a)if(f.call(a,b)){c[b]=a[b];d[b]=function(i){return function(){return this.each(c[i],arguments)}}(b)}}},constructor:function(a){if(typeof a==="string")a=this._find(document,a);else if(!a||a.nodeType||a.length===undefined||a===window)a=[a];for(var b=0,c=a.length;b<c;b++)if(a[b])this[this.length++]=a[b]},length:0,find:function(a){return new XNette.Q(this._find(this[0],a))},_find:function(a,b){if(!a||!b)return[];else if(document.querySelectorAll)return a.querySelectorAll(b);
else if(b.charAt(0)==="#")return[document.getElementById(b.substring(1))];else{b=b.split(".");var c=a.getElementsByTagName(b[0]||"*");if(b[1]){for(var d=[],f=RegExp("(^|\\s)"+b[1]+"(\\s|$)"),i=0,k=c.length;i<k;i++)f.test(c[i].className)&&d.push(c[i]);return d}else return c}},dom:function(){return this[0]},each:function(a,b){for(var c=0,d;c<this.length;c++)if((d=a.apply(this[c],b||[]))!==undefined)return d;return this}});var h=XNette.Q.factory,e=XNette.Q.implement;e({bind:function(a,b){if(document.addEventListener&&
...
(function(){XNette.Debug={};var h=XNette.Q.factory,e=XNette.Debug.Panel=XNette.Class({Extends:XNette.Q,Static:{PEEK:"nette-mode-peek",FLOAT:"nette-mode-float",WINDOW:"nette-mode-window",FOCUSED:"nette-focused",factory:function(a){return new e(a)},_toggle:function(a){var b=a.rel;b=b.charAt(0)==="#"?h(b):h(a)[b==="prev"?"prev":"next"](b.substring(4));if(b.css("display")==="none"){b.show();a.innerHTML=a.innerHTML.replace("►","▼")}else{b.hide();a.innerHTML=a.innerHTML.replace("▼","►")}}},constructor:function(a){XNette.Q.call(this,
...
<div class="nette-fixed-coords">
	<div id="nette-debug-bar">
		<ul>
			...
			. (class_exists('XNette\Framework') ? 'XNette Framework ' . XNette\Framework::VERSION . ' (' . substr(XNette\Framework::REVISION, 8) . ')' : '')) ?>">&nbsp;<span>XNette Framework</span></li>
...
(function (onloadOrig) {
	window.onload = function() {
		...
		var debug = document.getElementById('xnette-debug');
		document.body.appendChild(debug);
		debug.innerHTML = <?php echo json_encode(XNette\Utils\Strings::fixEncoding($output)) ?>;
		for (var i = 0, scripts = debug.getElementsByTagName('script'); i < scripts.length; i++) eval(scripts[i].innerHTML);
		(new XNette.Debug.Bar).init();
		XNette.Q.factory(debug).show();
	};
})(window.onload);
</script>
...
class BlueScreen extends XNette\Object
{
...
$title = ($exception instanceof XNette\FatalErrorException && isset($errorTypes[$exception->getSeverity()])) ? $errorTypes[$exception->getSeverity()] : get_class($exception);

$expandPath = XNETTE_DIR . DIRECTORY_SEPARATOR;
$counter = 0;
...
<head>
	...
	<meta name="generator" content="XNette Framework">
...
<div id="netteBluescreen">
	...
	><div>
		...
		<?php do { ?>
			...
			<div id="netteBsPnl<?php echo $counter ?>" class="<?php echo $collapsed ? 'xnette-collapsed ' : '' ?>inner">
				<div class="panel">
			...
			<div id="netteBsPnl<?php echo $counter ?>" class="<?php echo $collapsed ? 'xnette-collapsed ' : '' ?>inner">
				...
				<pre><?php echo self::highlightFile($ex->getFile(), $ex->getLine()) ?></pre>
			</div></div>
			<?php endif?>

			<?php if (isset($stack[0]['class']) && $stack[0]['class'] === 'XNette\Diagnostics\Debugger' && ($stack[0]['function'] === '_shutdownHandler' || $stack[0]['function'] === '_errorHandler')) unset($stack[0]) ?>
			<?php if ($stack): ?>
			<div class="panel">
		...
		<ul>
			...
			<?php if (class_exists('XNette\Framework')): ?><li><?php echo htmlSpecialChars('XNette Framework ' . XNette\Framework::VERSION) ?> <i>(revision <?php echo htmlSpecialChars(XNette\Framework::REVISION) ?>)</i></li><?php endif ?>
		</ul>
	</div>
</div>
...
<?php
	...
	static function highlightFile($file, $line, $count = 15)
	{
		if (function_exists('ini_set')) {
...
final class Debugger
{
	...
	static function _init()
	{
		...
		self::$blueScreen->addPanel(function($e) {
			if ($e instanceof XNette\Templating\FilterException) {
				return array(
					'tab' => 'Template',
	...
	static function enable($mode = NULL, $logDirectory = NULL, $email = NULL)
	{
		...
		if (self::$productionMode === self::DETECT) {
			if (class_exists('XNette\Environment')) {
				self::$productionMode = XNette\Environment::isProduction();
		...
		if (is_string($logDirectory)) {
			...
			if (self::$logDirectory === FALSE) {
				throw new XNette\DirectoryNotFoundException("Directory '$logDirectory' is not found.");
			}
		...
		if (function_exists('ini_set')) {
			...
			throw new XNette\NotSupportedException('Function ini_set() must be enabled.');
		}

		if ($email) {
			if (!is_string($email)) {
				throw new XNette\InvalidArgumentException('Email address must be a string.');
			}
			self::$email = $email;
		}
	...
	static function log($message, $priority = self::INFO)
	{
		if (self::$logDirectory === FALSE) {
			...
			throw new XNette\InvalidStateException('Logging directory is not specified in XNette\Diagnostics\Debugger::$logDirectory.');
		}

		if ($message instanceof \Exception) {
			$exception = $message;
			$message = "PHP Fatal error: "
				. ($message instanceof XNette\FatalErrorException
	...
	static function _shutdownHandler()
	{
		...
		if (isset($types[$error['type']])) {
			self::_exceptionHandler(new XNette\FatalErrorException($error['message'], 0, $error['type'], $error['file'], $error['line'], NULL), TRUE);
		}
	...
	static function _exceptionHandler(\Exception $exception, $drawBar = FALSE)
	{
		if (!headers_sent()) {
...
<?php
			...
			echo "\nXNette\\Debug FATAL ERROR: thrown ", get_class($e), ': ', $e->getMessage(),
	...
	static function _errorHandler($severity, $message, $file, $line, $context)
	{
		...
		if ($severity === E_RECOVERABLE_ERROR || $severity === E_USER_ERROR) {
			throw new XNette\FatalErrorException($message, 0, $severity, $file, $line, $context);

		} elseif (($severity & error_reporting()) !== $severity) {
			return FALSE;

		} elseif (self::$strictMode && !self::$productionMode) {
			self::_exceptionHandler(new XNette\FatalErrorException($message, 0, $severity, $file, $line, $context));
		}
...
class Logger extends XNette\Object
{
	const DEBUG = 'debug',
		INFO = 'info',
		WARNING = 'warning',
	...
	function log($message, $priority = self::INFO)
	{
		if (!is_dir($this->directory)) {
			throw new XNette\DirectoryNotFoundException("Directory '$this->directory' is not found or is not directory.");
		}
...
class FireLogger extends XNette\Object
{
	const DEBUG = 'debug',
		INFO = 'info',
		WARNING = 'warning',
	...
	static function log($message, $priority = self::DEBUG)
	{
		...
		if (isset($args[0]) && $args[0] instanceof \Exception) {
			$e = array_shift($args);
			$trace = $e->getTrace();
			if (isset($trace[0]['class']) && $trace[0]['class'] === 'XNette\Diagnostics\Debugger'
			...
			$trace = debug_backtrace();
			if (isset($trace[1]['class']) && $trace[1]['class'] === 'XNette\Diagnostics\Debugger'
	...
	private static function jsonDump(&$var, $level = 0)
	{
		if (is_bool($var) || is_null($var) || is_int($var) || is_float($var)) {
			...
			return XNette\Utils\Strings::fixEncoding($var);
...
final class DefaultBarPanel extends XNette\Object implements IBarPanel
{
	private $id;
...
<?php foreach ($data as $item): ?>
	...
	<tr class="<?php echo $i++ % 2 ? 'xnette-alt' : '' ?>">
...
class ControlGroup extends XNette\Object
{
	...
	function add()
	{
		foreach (func_get_args() as $num => $item) {
			if ($item instanceof IControl) {
				...
				throw new XNette\InvalidArgumentException("Only IFormControl items are allowed, the #$num parameter is invalid.");
			}
		}
		return $this;
	}
...
namespace XNette\Forms\Controls {

use XNette;use XNette\Forms\IControl;use XNette\Utils\Html;use XNette\Forms\Form;use XNette\Forms\Rule;

abstract class BaseControl extends XNette\ComponentModel\Component implements IControl
{
	...
	{
		$this->monitor('XNette\Forms\Form');
		parent::__construct();
		$this->control = Html::el('input');
		$this->label = Html::el('label');
		$this->caption = $caption;
		$this->rules = new XNette\Forms\Rules($this);
	}
	...
	function getHtmlName()
	{
		if ($this->htmlName === NULL) {
			$name = str_replace(self::NAME_SEPARATOR, '][', $this->lookupPath('XNette\Forms\Form'), $count);
			if ($count) {
	...
	function getHtmlId()
	{
		if ($this->htmlId === FALSE) {
			...
			$this->htmlId = sprintf(self::$idMask, $this->getForm()->getName(), $this->lookupPath('XNette\Forms\Form'));
		}
		return $this->htmlId;
	}
	...
	function loadHttpData()
	{
		...
		$this->setValue(XNette\Utils\Arrays::get($this->getForm()->getHttpData(), $path));
	}
	...
	function getControl()
	{
		...
		$control->data('xnette-rules', $rules ? $rules : NULL);
...
class HiddenField extends BaseControl
{
	...
	function getControl()
	{
		return parent::getControl()
			...
			->data('xnette-rules', NULL);
	}

}

class SubmitButton extends Button implements XNette\Forms\ISubmitterControl
{
	...
	static function validateSubmitted(XNette\Forms\ISubmitterControl $control)
	{
...
class ImageButton extends SubmitButton
{
	...
	function loadHttpData()
	{
		...
		$this->setValue(XNette\Utils\Arrays::get($this->getForm()->getHttpData(), $path) !== NULL);
	}
...
class SelectBox extends BaseControl
{
	...
	function setItems(array $items, $useKeys = TRUE)
	{
		...
		foreach ($items as $key => $value) {
			...
			foreach ($value as $key2 => $value2) {
				if (!$this->useKeys) {
					if (!is_scalar($value2)) {
						throw new XNette\InvalidArgumentException("All items must be scalar.");
					}
					$key2 = $value2;
				}

				if (isset($this->allowed[$key2])) {
					throw new XNette\InvalidArgumentException("Items contain duplication for key '$key2'.");
				}
	...
	function getControl()
	{
		...
		if ($this->skipFirst) {
			reset($this->items);
			$control->data('xnette-empty-value', $this->useKeys ? key($this->items) : current($this->items));
		}
		$selected = $this->getValue();
		$selected = is_array($selected) ? array_flip($selected) : array($selected => TRUE);
		$option = XNette\Utils\Html::el('option');
		...
		foreach ($this->items as $key => $value) {
			...
			foreach ($value as $key2 => $value2) {
				if ($value2 instanceof XNette\Utils\Html) {
...
class RadioList extends BaseControl
{
	...
	function getControl($key = NULL)
	{
		...
		foreach ($this->items as $k => $val) {
			...
			$control->data('xnette-rules', NULL);
...
abstract class TextBase extends BaseControl
{
	...
	{
		$this->filters[] = \XNette\callback($filter);
		return $this;
	}
	...
	function getControl()
	{
		...
		foreach ($this->getRules() as $rule) {
			if ($rule->type === XNette\Forms\Rule::VALIDATOR && !$rule->isNegative
		...
		if ($this->emptyValue !== '') {
			$control->data('xnette-empty-value', $this->translate($this->emptyValue));
		}
		return $control;
	}
	...
	function addRule($operation, $message = NULL, $arg = NULL)
	{
		if ($operation === Form::FLOAT) {
			$this->addFilter(\XNette\callback(__CLASS__, 'filterFloat'));
		}
...
class TextInput extends TextBase
{
	...
	function __construct($label = NULL, $cols = NULL, $maxLength = NULL)
	{
		...
		$this->filters[] = \XNette\callback($this, 'sanitize');
		$this->value = '';
	}

	function sanitize($value)
	{
		if ($this->control->maxlength && XNette\Utils\Strings::length($value) > $this->control->maxlength) {
			$value = iconv_substr($value, 0, $this->control->maxlength, 'UTF-8');
		}
		return XNette\Utils\Strings::trim(strtr($value, "\r\n", '  '));
	}
	...
	function getControl()
	{
		$control = parent::getControl();
		foreach ($this->getRules() as $rule) {
			if ($rule->isNegative || $rule->type !== XNette\Forms\Rule::VALIDATOR) {

			} elseif ($rule->operation === XNette\Forms\Form::RANGE && $control->type !== 'text') {
				list($control->min, $control->max) = $rule->arg;

			} elseif ($rule->operation === XNette\Forms\Form::PATTERN) {
...
class UploadControl extends BaseControl
{
	...
	protected function attached($form)
	{
		if ($form instanceof XNette\Forms\Form) {
			if ($form->getMethod() !== XNette\Forms\Form::POST) {
				throw new XNette\InvalidStateException('File upload requires method POST.');
			}
...
namespace XNette\Forms\Rendering {

use XNette;use XNette\Utils\Html;

class DefaultFormRenderer extends XNette\Object implements XNette\Forms\IFormRenderer
{
	...
	function render(XNette\Forms\Form $form, $mode = NULL)
	{
		if ($this->form !== $form) {
			$this->form = $form;
			$this->init();
	...
	function renderEnd()
	{
		$s = '';
		foreach ($this->form->getControls() as $control) {
			if ($control instanceof XNette\Forms\Controls\HiddenField && !$control->getOption('rendered')) {
	...
	function renderErrors(XNette\Forms\IControl $control = NULL)
	{
	...
	{
		if (!($parent instanceof XNette\Forms\Container || $parent instanceof XNette\Forms\ControlGroup)) {
			throw new XNette\InvalidArgumentException("Argument must be FormContainer or FormGroup instance.");
		}
		...
		foreach ($parent->getControls() as $control) {
			if ($control->getOption('rendered') || $control instanceof XNette\Forms\Controls\HiddenField || $control->getForm(FALSE) !== $this->form) {

			} elseif ($control instanceof XNette\Forms\Controls\Button) {
				$buttons[] = $control;
	...
	function renderPair(XNette\Forms\IControl $control)
	{
	...
	function renderPairMulti(array $controls)
	{
		...
		foreach ($controls as $control) {
			if (!$control instanceof XNette\Forms\IControl) {
				throw new XNette\InvalidArgumentException("Argument must be array of IFormControl instances.");
			}
			$s[] = (string) $control->getControl();
		}
		$pair = $this->getWrapper('pair container');
	...
	function renderLabel(XNette\Forms\IControl $control)
	{
		$head = $this->getWrapper('label container');

		if ($control instanceof XNette\Forms\Controls\Checkbox || $control instanceof XNette\Forms\Controls\Button) {
	...
	function renderControl(XNette\Forms\IControl $control)
	{
...
final class Rule extends XNette\Object
{
...
final class Rules extends XNette\Object implements \IteratorAggregate
{
	...
	private function adjustOperation($rule)
	{
		...
		if (!$this->getCallback($rule)->isCallable()) {
			...
			throw new XNette\InvalidArgumentException("Unknown operation$operation for control '{$rule->control->name}'.");
		}
	}
	...
	private function getCallback($rule)
	{
		...
		if (is_string($op) && strncmp($op, ':', 1) === 0) {
			return \XNette\callback(get_class($rule->control), self::VALIDATE_PREFIX . ltrim($op, ':'));
		} else {
			return \XNette\callback($op);
		}
	}
...
class Context extends XNette\Object
{
...
class FileUpload extends XNette\Object
{
	...
	function getContentType()
	{
		if ($this->isOk() && $this->type === NULL) {
			$this->type = XNette\Utils\MimeTypeDetector::fromFile($this->tmpName);
		}
		return $this->type;
	}
	...
	function move($dest)
	{
		...
		if (!$func($this->tmpName, $dest)) {
			throw new XNette\InvalidStateException("Unable to move uploaded file '$this->tmpName' to '$dest'.");
		}
		chmod($dest, 0644);
		$this->tmpName = $dest;
		return $this;
	...
	function toImage()
	{
		return XNette\Image::fromFile($this->tmpName);
	}
...
class Request extends XNette\Object implements IRequest
{
	...
	{
		$args = func_get_args();
		return XNette\Utils\Arrays::get($this->files, $args);
	}
...
class RequestFactory extends XNette\Object
{
...
final class Response extends XNette\Object implements IResponse
{
	...
	function setCode($code)
	{
		...
		if (!isset($allowed[$code])) {
			throw new XNette\InvalidArgumentException("Bad HTTP response '$code'.");

		} elseif (headers_sent($file, $line)) {
			throw new XNette\InvalidStateException("Cannot set HTTP code after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
	...
	function setHeader($name, $value)
	{
		if (headers_sent($file, $line)) {
			throw new XNette\InvalidStateException("Cannot send header after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
		}
	...
	function setExpiration($time)
	{
		...
		$time = XNette\DateTime::from($time);
	...
	function __destruct()
	{
		if (self::$fixIE && isset($_SERVER['HTTP_USER_AGENT']) && strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE ') !== FALSE
			...
			echo XNette\Utils\Strings::random(2e3, " \t\r\n");
			self::$fixIE = FALSE;
		}
	}
	...
	{
		if (headers_sent($file, $line)) {
			throw new XNette\InvalidStateException("Cannot set cookie after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
		}

		setcookie(
			$name,
			$value,
			$time ? XNette\DateTime::from($time)->format('U') : 0,
	...
	{
		if (headers_sent($file, $line)) {
			throw new XNette\InvalidStateException("Cannot delete cookie after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
		}
...
class Session extends XNette\Object
{
	...
	function start()
	{
		if (self::$started) {
			...
			throw new XNette\InvalidStateException('A session had already been started by session.auto-start or session_start().');
		}
		...
		if (XNette\Diagnostics\Debugger::catchError($e)) {
			@session_write_close();
			throw new XNette\InvalidStateException('session_start(): ' . $e->getMessage(), 0, $e);
		}
		...
		$browserKey = $this->request->getCookie('xnette-browser');
		if (!$browserKey) {
			$browserKey = XNette\Utils\Strings::random();
		}
				...
				if (is_array($metadata)) {
					foreach ($metadata as $variable => $value) {
						if ((!empty($value['B']) && $browserClosed) || (!empty($value['T']) && $now > $value['T'])
							|| ($variable !== '' && is_object($nf['DATA'][$namespace][$variable]) && (isset($value['V']) ? $value['V'] : NULL)
								!== XNette\Reflection\ClassType::from($nf['DATA'][$namespace][$variable])->getAnnotation('serializationVersion'))
						) {
							if ($variable === '') {
	...
	{
		if (!self::$started) {
			throw new XNette\InvalidStateException('Session is not started.');
		}
	...
	function regenerateId()
	{
		if (self::$started) {
			if (headers_sent($file, $line)) {
				throw new XNette\InvalidStateException("Cannot regenerate session ID after HTTP headers have been sent" . ($file ? " (output started at $file:$line)." : "."));
			}
			session_regenerate_id(TRUE);
	...
	function setName($name)
	{
		if (!is_string($name) || !preg_match('#[^0-9.][^.]*$#A', $name)) {
			throw new XNette\InvalidArgumentException('Session name must be a string and cannot contain dot.');
		}
	...
	function getNamespace($name, $class = 'XNette\Http\SessionNamespace')
	{
	...
	private function configure(array $config)
	{
		...
		foreach ($config as $key => $value) {
			...
			if ($value === NULL) {
				...
				if (self::$started) {
					throw new XNette\InvalidStateException("Unable to set '$key' when session has been started.");
				}
				$key = "session_$key";
				$key($value);
				...
				if (ini_get($key) != $value && !XNette\Framework::$iAmUsingBadHost) {
					throw new XNette\NotSupportedException('Required function ini_set() is disabled.');
				}
	...
	function setExpiration($time)
	{
		if (empty($time)) {
			...
			$time = XNette\DateTime::from($time)->format('U') - time();
			return $this->setOptions(array(
				'gc_maxlifetime' => $time,
	...
	{
		if (self::$started) {
			throw new XNette\InvalidStateException("Unable to set storage when session has been started.");
		}
	...
	private function sendCookie()
	{
		...
		$this->response->setCookie(
			...
			'xnette-browser', $_SESSION['__NF']['B'],
...
final class SessionNamespace extends XNette\Object implements \IteratorAggregate, \ArrayAccess
{
	...
	{
		if (!is_string($name)) {
			throw new XNette\InvalidArgumentException("Session namespace must be a string, " . gettype($name) ." given.");
		}
	...
	function __set($name, $value)
	{
		...
		if (is_object($value)) {
			$this->meta[$name]['V'] = XNette\Reflection\ClassType::from($value)->getAnnotation('serializationVersion');
		}
	}
	...
	function setExpiration($time, $variables = NULL)
	{
		...
		if (empty($time)) {
			...
			$time = XNette\DateTime::from($time)->format('U');
...
class Url extends XNette\FreezableObject
{
	...
	function __construct($url = NULL)
	{
		if (is_string($url)) {
			$parts = @parse_url($url);
			if ($parts === FALSE) {
				throw new XNette\InvalidArgumentException("Malformed or unsupported URI '$url'.");
			}
...
use XNette\Security\IAuthenticator;use XNette\Security\IAuthorizator;use XNette\Security\IIdentity;

class User extends XNette\Object implements IUser
{
	...
	final function getAuthenticationHandler()
	{
	...
	protected function getSessionNamespace($need)
	{
		...
		$this->session = $session = $this->context->session->getNamespace('XNette.Web.User/' . $this->namespace);
...
class CachingIterator extends \CachingIterator implements \Countable
{
	...
	function __construct($iterator)
	{
		if (is_array($iterator) || $iterator instanceof \stdClass) {
			...
			throw new XNette\InvalidArgumentException("Invalid argument passed to foreach resp. " . __CLASS__ . "; array or Traversable expected, " . (is_object($iterator) ? get_class($iterator) : gettype($iterator)) ." given.");
		}
	...
	function count()
	{
		...
		if ($inner instanceof \Countable) {
			...
			throw new XNette\NotSupportedException('Iterator is not countable.');
		}
	}
	...
	function __call($name, $args)
	{
		return XNette\ObjectMixin::call($this, $name, $args);
	}

	function &__get($name)
	{
		return XNette\ObjectMixin::get($this, $name);
	}

	function __set($name, $value)
	{
		return XNette\ObjectMixin::set($this, $name, $value);
	}

	function __isset($name)
	{
		return XNette\ObjectMixin::has($this, $name);
	}

	function __unset($name)
	{
		XNette\ObjectMixin::remove($this, $name);
	}
...
namespace XNette\Latte {

use XNette;use XNette\Utils\Strings;use XNette\Utils\Tokenizer;

class DefaultMacros extends XNette\Object
{
	...
	public static $defaultMacros = array(
		...
		'attr' => '<?php echo XNette\Utils\Html::el(NULL)->%:macroAttr%attributes() ?>',
	...
	function __construct()
	{
		$this->tokenizer = new Tokenizer(array(
			...
			self::T_COMMENT => '(?s)/\*.*?\*/',
			Parser::RE_STRING,
			'(?:true|false|null|and|or|xor|clone|new|instanceof|return|continue|break|[A-Z_][A-Z0-9_]{2,})(?![\d\pL_])',
			'\([a-z]+\)',
...
if ($_l->extends) {
	...
	return XNette\Latte\DefaultMacros::renderSnippets($control, $_l, get_defined_vars());
}
?>' . $s . '<?php
if ($_l->extends) {
	ob_end_clean();
	XNette\Latte\DefaultMacros::includeTemplate($_l->extends, get_defined_vars(), $template)->render();
}
';
		} else {
			$s = '<?php
...
?>' . $s;
				...
				$s = Strings::replace($s,
					...
					function ($matches) use($name, & $code, $uniq) {
						...
						$code = "//\n// block $name\n//\n"
							...
							. ($name[0] === '_' ? '; $control->validateControl(' . var_export(substr($name, 1), TRUE) . ')' : '')
							. "\n?>$content<?php\n}}";
						return '';
					});
			}
	...
	function macroInclude($content, $modifiers, $isDefinition = FALSE)
	{
		$destination = $this->fetchToken($content);
		...
		if ($destination === NULL) {
			...
			$cmd = isset($this->namedBlocks[$destination]) && !$parent
				...
				: 'XNette\Latte\DefaultMacros::callBlock' . ($parent ? 'Parent' : '') . "(\$_l, $name, $params)";
			return $modifiers
			...
			$cmd = 'XNette\Latte\DefaultMacros::includeTemplate(' . $destination . ', '
	...
	function macroBlock($content, $modifiers)
	{
		$name = $this->fetchToken($content);
	...
	function macroSnippet($content)
	{
		return $this->macroBlock('_' . $content, '');
	}

	function macroSnippetEnd($content)
	{
		return $this->macroBlockEnd('', '');
	}

	function macroCapture($content, $modifiers)
	{
		$name = $this->fetchToken($content);
	...
	function macroCaptureEnd($content)
	{
		list($type, $name, $modifiers) = array_pop($this->blocks);
		return $name . '=' . $this->formatModifiers('ob_get_clean()', $modifiers);
	}

	function macroCache($content)
	{
		return 'if (XNette\Latte\DefaultMacros::createCache($netteCacheStorage, '
			. var_export($this->uniq . ':' . $this->cacheCounter++, TRUE)
			. ', $_l->g->caches' . $this->formatArray($content, ', ') . ')) {';
	}

	function macroForeach($content)
	{
		return '$iterator = $_l->its[] = new XNette\Iterators\CachingIterator('
			. preg_replace('#(.*)\s+as\s+#i', '$1) as ', $this->formatMacroArgs($content), 1);
	}

	function macroIfset($content)
	{
	...
	function macroAttr($content)
	{
		return Strings::replace($content . ' ', '#\)\s+#', ')->');
	}

	function macroContentType($content)
	{
		if (strpos($content, 'html') !== FALSE) {
			$this->parser->escape = 'XNette\Templating\DefaultHelpers::escapeHtml|';
			$this->parser->context = Parser::CONTEXT_TEXT;

		} elseif (strpos($content, 'xml') !== FALSE) {
			$this->parser->escape = 'XNette\Templating\DefaultHelpers::escapeXml';
			$this->parser->context = Parser::CONTEXT_NONE;

		} elseif (strpos($content, 'javascript') !== FALSE) {
			$this->parser->escape = 'XNette\Templating\DefaultHelpers::escapeJs';
			$this->parser->context = Parser::CONTEXT_NONE;

		} elseif (strpos($content, 'css') !== FALSE) {
			$this->parser->escape = 'XNette\Templating\DefaultHelpers::escapeCss';
			$this->parser->context = Parser::CONTEXT_NONE;
	...
	function macroDump($content)
	{
		return 'XNette\Diagnostics\Debugger::barDump('
	...
	function macroControl($content)
	{
		$pair = $this->fetchToken($content);
		if ($pair === NULL) {
		...
		return ($name[0] === '$' ? "if (is_object($name)) \$_ctrl = $name; else " : '')
			...
			. 'if ($_ctrl instanceof XNette\Application\UI\IPartiallyRenderable) $_ctrl->validateControl(); '
			. "\$_ctrl->$method($param)";
	}

	function macroLink($content, $modifiers)
	{
		return $this->formatModifiers('$control->link(' . $this->formatLink($content) .')', $modifiers);
	}

	function macroPlink($content, $modifiers)
	{
		return $this->formatModifiers('$presenter->link(' . $this->formatLink($content) .')', $modifiers);
	}

	function macroIfCurrent($content)
	{
		return ($content ? 'try { $presenter->link(' . $this->formatLink($content) . '); } catch (XNette\Application\UI\InvalidLinkException $e) {}' : '')
			. '; if ($presenter->getLastCreatedRequestFlag("current")):';
	}

	private function formatLink($content)
	{
		return $this->formatString($this->fetchToken($content)) . $this->formatArray($content, ', ');
	}

	function macroVar($content, $modifiers, $extract = FALSE)
	{
		$out = '';
		$var = TRUE;
	...
	function macroDefault($content)
	{
		return 'extract(array(' . $this->macroVar($content, '', TRUE) . '), EXTR_SKIP)';
	}

	function macroModifiers($content, $modifiers)
	{
	...
	function formatModifiers($var, $modifiers)
	{
		if (!$modifiers) {
			return $var;
		}
	...
	function fetchToken(& $s)
	{
		if ($matches = Strings::match($s, '#^((?>'.Parser::RE_STRING.'|[^\'"\s,]+)+)\s*,?\s*(.*)$#s')) {
			$s = $matches[2];
			return $matches[1];
		}
		return NULL;
	...
	function formatMacroArgs($input)
	{
		$out = '';
	...
	function formatArray($input, $prefix = '')
	{
	...
	static function callBlock($context, $name, $params)
	{
		if (empty($context->blocks[$name])) {
			throw new XNette\InvalidStateException("Cannot include undefined block '$name'.");
		}
	...
	static function callBlockParent($context, $name, $params)
	{
		if (empty($context->blocks[$name]) || ($block = next($context->blocks[$name])) === FALSE) {
			throw new XNette\InvalidStateException("Cannot include undefined parent block '$name'.");
		}
		$block($context, $params);
	}
	...
	{
		if ($destination instanceof XNette\Templating\ITemplate) {
			$tpl = $destination;

		} elseif ($destination == NULL) {
			throw new XNette\InvalidArgumentException("Template file name was not specified.");

		} else {
			$tpl = clone $template;
			if ($template instanceof XNette\Templating\IFileTemplate) {
	...
	static function initRuntime($template, $extends, $realFile)
	{
		$local = (object) NULL;
		...
		if (!empty($local->g->caches)) {
			end($local->g->caches)->dependencies[XNette\Caching\Cache::FILES][] = $template->getFile();
		}
	...
	static function renderSnippets($control, $local, $params)
	{
		...
		if ($control instanceof XNette\Application\UI\Control) {
			foreach ($control->getComponents(FALSE, 'XNette\Application\UI\Control') as $child) {
	...
	static function createCache(XNette\Caching\IStorage $cacheStorage, $key, & $parents, $args = NULL)
	{
		if ($args) {
		...
		if ($parents) {
			end($parents)->dependencies[XNette\Caching\Cache::ITEMS][] = $key;
		}

		$cache = new XNette\Caching\Cache($cacheStorage, 'XNette.Templating.Cache');
		if ($helper = $cache->start($key)) {
			$helper->dependencies = array(
				XNette\Caching\Cache::TAGS => isset($args['tags']) ? $args['tags'] : NULL,
				XNette\Caching\Cache::EXPIRATION => isset($args['expire']) ? $args['expire'] : '+ 7 days',
			);
			$parents[] = $helper;
		}
		return $helper;
...
class Engine extends XNette\Object
{
...
class HtmlNode extends XNette\Object
{
	...
	{
		$this->name = $name;
		$this->isEmpty = isset(XNette\Utils\Html::$emptyElements[strtolower($this->name)]);
	}

}

class MacroNode extends XNette\Object
{
	...
	function __construct($name, $args = NULL, $modifiers = NULL)
	{
		$this->name = $name;
		$this->args = $args;
...
class FilterException extends XNette\InvalidStateException
{
...
class ParseException extends XNette\Templating\FilterException
{
}

use XNette\Utils\Strings;

class Parser extends XNette\Object
{
	...
	private function contextText()
	{
		...
		if (!$matches || !empty($matches['macro']) || !empty($matches['comment'])) {
			...
			$this->escape = 'XNette\Templating\DefaultHelpers::escapeHtmlComment';
			...
			$this->escape = 'XNette\Templating\DefaultHelpers::escapeHtml';
	...
	private function contextTag()
	{
		...
		if (!$matches || !empty($matches['macro']) || !empty($matches['comment'])) {
			...
			if ($isEmpty) {
				$matches[0] = (XNette\Utils\Html::$xhtml ? ' />' : '>')
			...
			if (!$node->closing && (strcasecmp($node->name, 'script') === 0 || strcasecmp($node->name, 'style') === 0)) {
				...
				$this->escape = 'XNette\Templating\DefaultHelpers::escape' . (strcasecmp($node->name, 'style') ? 'Js' : 'Css');
			} else {
			...
			if ($isSpecial || $node instanceof MacroNode) {
				...
				$this->escape = strncasecmp($name, 'on', 2)
					? ('XNette\Templating\DefaultHelpers::escapeHtml' . (strcasecmp($name, 'style') ? "|$value" : 'Css'))
					: "XNette\\Templating\\DefaultHelpers::escapeHtmlJs|$value";
			}
		}
		return $matches;
	}
	...
	function macro($name, $args = '', $modifiers = '')
	{
		...
		return Strings::replace(
			...
			function ($m) use($handler, $node) {
				if ($m[1]) {
					return \XNette\callback($m[1][0] === ':' ? array($handler, substr($m[1], 1)) : $m[1])
	...
	function tagMacro($name, $attrs, $closing)
	{
		$knownTags = array(
			'include' => 'block',
			'for' => 'each',
	...
	function attrsMacro($code, $attrs, $closing)
	{
...
use XNette;use XNette\Utils\Strings;use XNette\Caching\Cache;
...
class RobotLoader extends AutoLoader
{
	...
	function __construct()
	{
		if (!extension_loaded('tokenizer')) {
			throw new XNette\NotSupportedException("PHP extension Tokenizer is not loaded.");
		}
	}
	...
	function register()
	{
		...
		if (isset($this->list[strtolower(__CLASS__)]) && class_exists('XNette\Loaders\XNetteLoader', FALSE)) {
			XNetteLoader::getInstance()->unregister();
		}
	...
	function tryLoad($type)
	{
			...
			if ($initiator === 'class_exists' || $initiator === 'interface_exists') {
				...
				if ($this->autoRebuild && $this->rebuilt) {
					$this->getCache()->save($this->getKey(), $this->list, array(
						Cache::CONSTS => 'XNette\Framework::REVISION',
					));
				}
			}
		...
		if (isset($this->list[$type][0])) {
			XNette\Utils\LimitedScope::load($this->list[$type][0]);
			self::$count++;
		}
	}
	...
	function rebuild()
	{
		$this->getCache()->save($this->getKey(), \XNette\callback($this, '_rebuildCallback'), array(
			Cache::CONSTS => 'XNette\Framework::REVISION',
		));
		$this->rebuilt = TRUE;
	}
	...
	function addDirectory($path)
	{
		foreach ((array) $path as $val) {
			$real = realpath($val);
			if ($real === FALSE) {
				throw new XNette\DirectoryNotFoundException("Directory '$val' not found.");
			}
			$this->scanDirs[] = $real;
		}
		return $this;
	...
	private function addClass($class, $file, $time)
	{
		...
		if (isset($this->list[$lClass][0]) && ($file2 = $this->list[$lClass][0]) !== $file && is_file($file2)) {
			...
			$e = new XNette\InvalidStateException("Ambiguous class '$class' resolution; defined in $file and in " . $this->list[$lClass][0] . ".");
			{
				throw $e;
			}
		}
	...
	private function scanDirectory($dir)
	{
		if (is_dir($dir)) {
			$disallow = array();
			$iterator = XNette\Utils\Finder::findFiles(Strings::split($this->acceptFiles, '#[,\s]+#'))
	...
	function setCacheStorage(XNette\Caching\IStorage $storage)
	{
	...
	protected function getCache()
	{
		if (!$this->cacheStorage) {
			...
			$this->cacheStorage = new XNette\Caching\Storages\DevNullStorage;
		}
		return new Cache($this->cacheStorage, 'XNette.RobotLoader');
	}
...
use XNette;use XNette\Utils\Strings;

class MimePart extends XNette\Object
{
	...
	function setHeader($name, $value, $append = FALSE)
	{
		if (!$name || preg_match('#[^a-z0-9-]#i', $name)) {
			throw new XNette\InvalidArgumentException("Header name must be non-empty alphanumeric string, '$name' given.");
		}
		...
		if ($value == NULL) {
			...
			foreach ($value as $email => $name) {
				if ($name !== NULL && !Strings::checkEncoding($name)) {
					throw new XNette\InvalidArgumentException("Name is not valid UTF-8 string.");
				}

				if (!preg_match('#^[^@",\s]+@[^@",\s]+\.[a-z]{2,10}$#i', $email)) {
					throw new XNette\InvalidArgumentException("Email address '$email' is not valid.");
				}

				if (preg_match('#[\r\n]#', $name)) {
					throw new XNette\InvalidArgumentException("Name must not contain line separator.");
				}
				$tmp[$email] = $name;
			}
			...
			if (!Strings::checkEncoding($value)) {
				throw new XNette\InvalidArgumentException("Header is not valid UTF-8 string.");
			}
	...
	function generateMessage()
	{
		...
		if ($body !== '') {
			...
			default:
				throw new XNette\InvalidStateException('Unknown encoding.');
			}
		}
...
class Message extends MimePart
{
	...
	public static $defaultMailer = 'XNette\Mail\SendmailMailer';
	...
	private function createAttachment($file, $content, $contentType, $disposition)
	{
		...
		if ($content === NULL) {
			...
			if ($content === FALSE) {
				throw new XNette\FileNotFoundException("Unable to read file '$file'.");
			}
		} else {
			$content = (string) $content;
		}
		$part->setBody($content);
		$part->setContentType($contentType ? $contentType : XNette\Utils\MimeTypeDetector::fromString($content));
	...
	{
		if ($this->html instanceof XNette\Templating\ITemplate) {
			$this->html->mail = $this;
			if ($this->basePath === NULL && $this->html instanceof XNette\Templating\IFileTemplate) {
	...
	{
		$text = $this->getBody();
		if ($text instanceof XNette\Templating\ITemplate) {
			$text->mail = $this;
...
class SendmailMailer extends XNette\Object implements IMailer
{
	...
	function send(Message $mail)
	{
		...
		if (XNette\Diagnostics\Debugger::catchError($e)) {
			throw new XNette\InvalidStateException('mail(): ' . $e->getMessage(), 0, $e);

		} elseif (!$res) {
			throw new XNette\InvalidStateException('Unable to send email.');
		}
	}

}

class SmtpMailer extends XNette\Object implements IMailer
{
...
class Annotation extends XNette\Object implements IAnnotation
{
...
final class AnnotationsParser
{
	...
	static function getAll(\Reflector $r)
	{
		...
		if (self::$useReflection) {
			...
			if (!self::$cacheStorage) {
				...
				self::$cacheStorage = new XNette\Caching\Storages\DevNullStorage;
			}
			$outerCache = new XNette\Caching\Cache(self::$cacheStorage, 'XNette.Reflection.Annotations');
	...
	static function setCacheStorage(XNette\Caching\IStorage $storage)
	{
...
class GlobalFunction extends \ReflectionFunction
{
	...
	function toCallback()
	{
		return new XNette\Callback($this->value);
	}
...
class Method extends \ReflectionMethod
{
	...
	function toCallback()
	{
		return new XNette\Callback(parent::getDeclaringClass()->getName(), $this->getName());
	}
...
class Parameter extends \ReflectionParameter
{
	...
	{
		return ($tmp = XNette\Utils\Strings::match($this, '#>\s+([a-z0-9_\\\\]+)#i')) ? $tmp[1] : NULL;
	}
...
class Identity extends XNette\FreezableObject implements IIdentity
{
...
class Permission extends XNette\Object implements IAuthorizator
{
	...
	function addRole($role, $parents = NULL)
	{
		...
		if (isset($this->roles[$role])) {
			throw new XNette\InvalidStateException("Role '$role' already exists in the list.");
		}
	...
	private function checkRole($role, $need = TRUE)
	{
		if (!is_string($role) || $role === '') {
			throw new XNette\InvalidArgumentException("Role must be a non-empty string.");

		} elseif ($need && !isset($this->roles[$role])) {
			throw new XNette\InvalidStateException("Role '$role' does not exist.");
		}
	}
	...
	function addResource($resource, $parent = NULL)
	{
		...
		if (isset($this->resources[$resource])) {
			throw new XNette\InvalidStateException("Resource '$resource' already exists in the list.");
		}
	...
	private function checkResource($resource, $need = TRUE)
	{
		if (!is_string($resource) || $resource === '') {
			throw new XNette\InvalidArgumentException("Resource must be a non-empty string.");

		} elseif ($need && !isset($this->resources[$resource])) {
			throw new XNette\InvalidStateException("Resource '$resource' does not exist.");
		}
	}
	...
	protected function setRule($toAdd, $type, $roles, $resources, $privileges, $assertion = NULL)
	{
		...
		$assertion = $assertion ? \XNette\callback($assertion) : NULL;
...
class SimpleAuthenticator extends XNette\Object implements IAuthenticator
{
...
use XNette;use XNette\Utils\Strings;use XNette\Forms\Form;use XNette\Utils\Html;
...
final class DefaultHelpers
{
	...
	{
		$callback = \XNette\callback('XNette\Templating\DefaultHelpers', $helper);
		if ($callback->isCallable()) {
			return $callback;
		}
		$callback = \XNette\callback('XNette\Utils\Strings', $helper);
		if ($callback->isCallable()) {
			return $callback;
		}
	}

	static function escapeHtml($s, $quotes = NULL)
	{
	...
	static function escapeHtmlCss($s)
	{
	...
	static function escapeJs($s)
	{
		...
		return str_replace(']]>', ']]\x3E', XNette\Utils\Json::encode($s));
	}

	static function escapeHtmlJs($s, $quotes = NULL)
	{
	...
	{
		if ($type === NULL) {
			$type = XNette\Utils\MimeTypeDetector::fromString($data, NULL);
		}
...
abstract class Template extends XNette\Object implements ITemplate
{
	...
	{
		$callback = \XNette\callback($callback);
		if (in_array($callback, $this->filters)) {
			throw new XNette\InvalidStateException("Filter '$callback' was registered twice.");
		}
	...
	function save($file)
	{
		if (file_put_contents($file, $this->__toString(TRUE)) === FALSE) {
			throw new XNette\IOException("Unable to save file '$file'.");
		}
	}
	...
	{
		$this->helpers[strtolower($name)] = \XNette\callback($callback);
	}

	function registerHelperLoader($callback)
	{
		$this->helperLoaders[] = \XNette\callback($callback);
	}
	...
	function add($name, $value)
	{
		if (array_key_exists($name, $this->params)) {
			throw new XNette\InvalidStateException("The variable '$name' already exists.");
		}
	...
	static function optimizePhp($source)
	{
		$res = $php = '';
		$lastChar = ';';
...
class FileTemplate extends Template implements IFileTemplate
{
	...
	function setFile($file)
	{
		...
		if (!$this->file) {
			throw new XNette\FileNotFoundException("Missing template file '$file'.");
		}
		return $this;
	}
	...
	{
		if ($this->file == NULL) {
			throw new XNette\InvalidStateException("Template file name was not specified.");
		}

		$this->__set('template', $this);

		$cache = new Caching\Cache($storage = $this->getCacheStorage(), 'XNette.FileTemplate');
		...
		if ($content === NULL) {
			...
			$cache->save(
				...
				array(
					...
					Caching\Cache::CONSTS => 'XNette\Framework::REVISION',
				)
			);
			$cache->release();
		...
		if ($cached !== NULL && $storage instanceof Caching\Storages\PhpFileStorage) {
			XNette\Utils\LimitedScope::load($cached['file'], $this->getParams());
			flock($cached['handle'], LOCK_UN);
			fclose($cached['handle']);

		} else {
			XNette\Utils\LimitedScope::evaluate($content, $this->getParams());
		}
	}
	...
	static function & getRef(& $arr, $key)
	{
		foreach (is_array($key) ? $key : array($key) as $k) {
			if (is_array($arr) || $arr === NULL) {
				...
				throw new XNette\InvalidArgumentException('Traversed item is not an array.');
			}
		}
		return $arr;
	}
...
final class CriticalSection
{
	...
	static function enter()
	{
		if (self::$criticalSections) {
			throw new XNette\InvalidStateException('Critical section has already been entered.');
		}

		$handle = substr(PHP_OS, 0, 3) === 'WIN'
			? @fopen(XNETTE_DIR . '/lockfile', 'w')
			: @fopen(__FILE__, 'r');

		if (!$handle) {
			throw new XNette\InvalidStateException("Unable initialize critical section.");
		}
	...
	{
		if (!self::$criticalSections) {
			throw new XNette\InvalidStateException('Critical section has not been initialized.');
		}
...
class Finder extends XNette\Object implements \IteratorAggregate
{
	...
	{
		if ($this->paths) {
			throw new XNette\InvalidStateException('Directory to search has already been specified.');
		}
		if (!is_array($path)) {
			$path = func_get_args();
		}
	...
	{
		if (!$this->paths) {
			throw new XNette\InvalidStateException('Call in() or from() to specify directory to search.');
	...
	private function buildIterator($path)
	{
		if (PHP_VERSION_ID < 50301) {
			$iterator = new XNette\Utils\RecursiveDirectoryIteratorFixed($path);
		} else {
		...
		if ($this->exclude) {
			...
			$iterator = new XNette\Iterators\RecursiveFilter($iterator, function($file) use($filters) {
				if (!$file->isFile()) {
		...
		if ($this->groups) {
			$groups = $this->groups;
			$iterator = new XNette\Iterators\Filter($iterator, function($file) use($groups) {
	...
	function size($operator, $size = NULL)
	{
		if (func_num_args() === 1) {
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?((?:\d*\.)?\d+)\s*(K|M|G|)B?$#i', $operator, $matches)) {
				throw new XNette\InvalidArgumentException('Invalid size predicate format.');
			}
	...
	function date($operator, $date = NULL)
	{
		if (func_num_args() === 1) {
			if (!preg_match('#^(?:([=<>!]=?|<>)\s*)?(.+)$#i', $operator, $matches)) {
				throw new XNette\InvalidArgumentException('Invalid date predicate format.');
			}
			list(, $operator, $date) = $matches;
			$operator = $operator ? $operator : '=';
		}
		$date = XNette\DateTime::from($date)->format('U');
	...
	static function compare($l, $operator, $r)
	{
		...
		throw new XNette\InvalidArgumentException("Unknown operator $operator.");
	}
...
class Html extends XNette\Object implements \ArrayAccess, \Countable, \IteratorAggregate
{
	...
	final function setName($name, $isEmpty = NULL)
	{
		if ($name !== NULL && !is_string($name)) {
			throw new XNette\InvalidArgumentException("Name must be string or NULL, " . gettype($name) ." given.");
		}
	...
	final function setHtml($html)
	{
		if ($html === NULL) {
			...
			throw new XNette\InvalidArgumentException("Textual content must be a scalar, " . gettype($html) ." given.");
	...
	function insert($index, $child, $replace = FALSE)
	{
		if ($child instanceof Html || is_scalar($child)) {
			...
			throw new XNette\InvalidArgumentException("Child node must be scalar or Html object, " . (is_object($child) ? get_class($child) : gettype($child)) ." given.");
		}
	...
	final function getIterator($deep = FALSE)
	{
		if ($deep) {
			$deep = $deep > 0 ? \RecursiveIteratorIterator::SELF_FIRST : \RecursiveIteratorIterator::CHILD_FIRST;
			return new \RecursiveIteratorIterator(new XNette\Iterators\Recursor(new \ArrayIterator($this->children)), $deep);

		} else {
			return new XNette\Iterators\Recursor(new \ArrayIterator($this->children));
		}
	}
...
final class MimeTypeDetector
{
	...
	{
		if (!is_file($file)) {
			throw new XNette\FileNotFoundException("File '$file' not found.");
		}
...
class Neon extends XNette\Object
{
	const BLOCK = 1;
	...
	{
		if (!is_string($input)) {
			throw new XNette\InvalidArgumentException("Argument must be a string, " . gettype($input) . " given.");
		}
		if (!self::$tokenizer) {
...
class Paginator extends XNette\Object
{
...
class Strings
{
	...
	static function replace($subject, $pattern, $replacement = NULL, $limit = -1)
	{
		...
		if (is_object($replacement) || is_array($replacement)) {
			if ($replacement instanceof XNette\Callback) {
				$replacement = $replacement->getNative();
			}
			if (!is_callable($replacement, FALSE, $textual)) {
				XNette\Diagnostics\Debugger::catchError($foo);
				throw new XNette\InvalidStateException("Callback '$textual' is not callable.");
			}
...
class Tokenizer extends XNette\Object
{
...
namespace XNette {
 ...
 # XNette\Utils\SafeStream::register();
...
function dump($var)
{
	foreach (func_get_args() as $arg) Diagnostics\Debugger::dump($arg);
	return $var;
}
}